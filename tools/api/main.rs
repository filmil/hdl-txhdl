...
fn main() {
    ...
    // Module docs.
    for l in &lines {
        if l.starts_with("//!") { out.push_str(l); out.push('\n') } else if !l.is_empty() { break }
    }
    let mut i = 0;
...
enum Kind { Module, Trait, Impl, TraitImpl, Struct, Enum }

/// Emit the items of one block at `indent`, from line `*i` to the line
/// holding only `}` at that indent (or the end of the file).
fn items(lines: &[&str], i: &mut usize, indent: usize, kind: Kind, out: &mut String) {
    let pad = " ".repeat(indent);
    let mut docs = String::new();
    let mut attrs = String::new();
    while *i < lines.len() {
        let line = lines[*i];
        if indent > 0 && line == format!("{}}}", " ".repeat(indent - 4)) { return }
        let t = line.trim_start();
        if t.is_empty() || !line.starts_with(&pad) || line.len() - t.len() != indent {
            *i += 1;
            continue;
        }
        if t.starts_with("///") { docs.push_str(line); docs.push('\n'); *i += 1; continue }
        if t.starts_with("#[") { attrs.push_str(line); attrs.push('\n'); *i += 1; continue }
        ...
        loop {
            ...
            *i += 1;
            let e = l.trim_end();
            if e.ends_with('{') || e.ends_with(';') || e.ends_with(',') || e.ends_with("{}")
                || (e.contains('{') && e.ends_with('}')) || *i >= lines.len() { break }
            sig.push('\n');
        }
        let e = sig.trim_end();
        ...
        let public = t.starts_with("pub ") || t.starts_with("impl") || kind == Kind::Trait
            || kind == Kind::TraitImpl || kind == Kind::Enum
            || (t.starts_with("macro_rules!") && attrs.contains("macro_export"));
        let child = if t.starts_with("pub trait") || t.starts_with("trait ") { Some(Kind::Trait) }
            else if t.starts_with("impl") && t.contains(" for ") { Some(Kind::TraitImpl) }
            else if t.starts_with("impl") { Some(Kind::Impl) }
            else if t.starts_with("pub struct") || t.starts_with("struct ") { Some(Kind::Struct) }
            else if t.starts_with("pub enum") || t.starts_with("enum ") { Some(Kind::Enum) }
            else if t.starts_with("pub mod") || t.starts_with("mod ") { Some(Kind::Module) }
            else { None };
        if !public {
            docs.clear(); attrs.clear();
            if has_block { skip_body(lines, i, indent) }
            continue;
        }
        ...
        match (child, has_block) {
            (Some(k), true) => {
                ...
                *i += 1; // the closing brace
                ...
                if inherent && inner.trim().is_empty() {
                    ...
                    out.push_str(&format!("{docs}{attrs}{} {body}\n\n", head.trim_end_matches('{').trim_end()));
                } else {
            ...
            (Some(_), false) => {
                ...
                out.push_str(&format!("{docs}{attrs}{head}\n"));
                if kind == Kind::Module { out.push('\n') }
            }
            (None, _) => {
                let _ = start;
                out.push_str(&format!("{docs}{attrs}{head}\n"));
                if has_block && !one_line_body { skip_body(lines, i, indent) }
                if t.starts_with("macro_rules!") { out.push('\n') }
            }
        }
        docs.clear(); attrs.clear();
    }
...
fn signature(e: &str, kind: Kind) -> String {
    ...
    if (t.starts_with("pub struct") || t.starts_with("struct")) && !e.contains(" {") {
        if let Some(p) = paren_at_depth0(e) {
            if !e[p..].contains("pub ") { return format!("{}(..);", &e[..p]) }
        }
    }
    if e.ends_with('{') {
        let s = e.trim_end_matches('{').trim_end();
        return if matches!(kind, Kind::Trait | Kind::Impl | Kind::TraitImpl | Kind::Module) && (t.starts_with("pub fn") || t.starts_with("fn") || t.starts_with("pub async fn") || t.starts_with("async fn"))
            || (kind == Kind::Module && !(t.starts_with("pub trait") || t.starts_with("pub struct") || t.starts_with("pub enum") || t.starts_with("pub mod") || t.starts_with("impl")))
        { format!("{s};") } else { format!("{s} {{") };
    }
    if e.contains('{') && e.ends_with('}') {
        // `fn f() { .. }` on one line, or `impl X for Y {}`.
        let s = e.split('{').next().unwrap().trim_end();
        let block = t.starts_with("impl") || t.starts_with("pub trait") || t.starts_with("pub enum");
        return if block { format!("{s} {{}}") } else if t.starts_with("pub struct") { format!("{s} {{ .. }}") } else { format!("{s};") };
    }
    e.to_string()
}
...
fn paren_at_depth0(s: &str) -> Option<usize> {
    ...
    for (i, c) in s.char_indices() {
        match c { '<' => d += 1, '>' => d -= 1, '(' if d == 0 => return Some(i), _ => {} }
    }
    None
}
