...
\begin{itemize}
  ...
  \item a receive takes in its state alone, so \code{ready} is the
        state's condition;
  \item an output port keeps what a state set until a state sets it
        again, as the run's wire does: the lowering holds it in a
        register the unit does not declare, and the port is the
        state's expression while the state is about to leave and the
        register otherwise, which Section~\ref{sec:x-hold} shows;
