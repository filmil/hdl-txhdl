...
arrives and held inside the group until B's arrives, A's channel
buffers the next offers meanwhile, two deep, and when it is full A
sees \code{ready} low and waits, and so A runs at B's rate with no
code for the backpressure. Figure~\ref{fig:x-join} shows it:
\code{a\_valid} stays high while the channel holds a word, and a sum
lands on every edge B delivers.

\begin{figure}[htbp]
\centering
\input{join_timing.tex}
\caption{The join. A's channel holds a word for the group while B's arrives every other cycle; the sum follows B.}
\label{fig:x-join}
\end{figure}
