...
The sign extension of a value of one bit is the same disagreement
again. A wider value is extended by repeating its top bit, which
Verilog takes with a bit select and VHDL with \code{resize} on a
\code{signed}; neither works on one bit, since a wire of one bit is a
scalar in Verilog, with no bit to select, and a \code{std\_logic} in
VHDL, which \code{signed} may not convert. Such a value is its own top
bit, so the lowering repeats the value itself, \code{\{\{7\{odd\}\},
odd\}} and \code{unsigned'(7 downto 0 => odd)}. That was issue 247,
and \code{spread} below is the register that showed it.
...
which either input has its top bit set, spreads one computed bit over
a width in an eighth register, and says on two wires whether
...
\caption{The operators: the inputs, the sum, the two counts, the
spread bit, and the two compares.}
\label{fig:x-ops}
\end{figure}
