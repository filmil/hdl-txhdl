...
refresh is written to. The six are declared once, with \code{regmap!},
which writes the word a read answers, the word a write goes to, and
the offsets the example addresses the watchdog by (issue 670).
When the count reaches zero it asks the system control block of Section~\ref{sec:x-syscon} for a reset, which records
