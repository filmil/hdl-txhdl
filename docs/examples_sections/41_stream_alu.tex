...
and the value on another. The request is the line a station of
three inputs sends, its cells the operation and the two operands,
so the next example puts a station in front of this unit. The four operations are four units of four
...
\lstinputlisting[caption={\code{stream\_alu.rs}: the unit, a library both this example and the next run.},label={lst:x-stream-alu}]{lib/examples/stream_alu.rs}

\lstinputlisting[caption={\code{ex\_stream\_alu.rs}. Run with \code{bazel run //lib/examples:ex\_stream\_alu}.},label={lst:x-stream-alu-run}]{lib/examples/ex_stream_alu.rs}
