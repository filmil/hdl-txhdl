...
\code{let}, a \code{when!}, a \code{case!} or another \code{if}. The
...
\code{when!} remains the two-arm form with the arrow, and the choice
between the two is spelling: \code{if} where the arms exclude each
other and nest, \code{when!} where a drive is under one condition.
