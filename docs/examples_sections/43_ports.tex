...
work for \code{\#[lower]}. It reads the struct from the file the unit
is in, and makes each field a port under the field's name, in the
...
The struct must be declared in the same file as the unit.
\code{\#[lower]} sees one item at a time and reads the file for
anything else, as it does for the functions it inlines. A side whose
type is neither a port nor such a struct is refused with a message
naming the type, and so is a port name that two sides both use.
