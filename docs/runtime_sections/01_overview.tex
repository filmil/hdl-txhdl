...
\textbf{Every \code{.await} is a cycle boundary.} A second register
read written after a first waits for the next edge, as an operator
does. Reads that share one edge are written inside \code{parallel!},
which polls its futures together and returns their outputs as a tuple;
that is the one case the executor treats specially. A read polled
inside a \code{parallel!} whose process has already crossed the edge
at this step is free, and only once per register, so the group is one
wait and a \code{loop} advances one iteration per group. The first
version of the executor charged a cycle for every read, so an
iteration that read two registers took two cycles; the second made a
sequential read free, which was right in effect and said nothing in the
source. \code{parallel!} says it.
...
\code{parallel!} keeps its futures in the one process, which is the
difference between the two: \code{join2} forks, \code{parallel!} does
not.
