...
among them, VHDL's without regard to case. The module's own name is
a string given to \code{lowered} when it runs, which the macro never
sees, so each emitter refuses one its own language reserves as it
writes it: a unit may be lowered under a name only VHDL reserves and
still have its Verilog, or its tables drawn (issue 485). The
two word lists are one file, \code{lib/src/reserved.rs}, below: the
runtime compiles it as a module of its own, and the macro crate,
which cannot depend on the runtime since the runtime depends on it,
includes the same file by path. A computed \code{let} or a
