...
state: it reads \code{run} as one loop, or several under
...
value}, the arrow pointing into the register, or in \code{when!} a
channel's \code{send}, made under the predicate, and a statement
that is neither is refused with the macro's own message. Both are procedural
