...
reads registers and ports into names, predicates drives with
\code{when!} and \code{case!}, chooses values with \code{select!},
...
\code{concat}, \code{sext} and \code{zext} by their turbofish; a
struct literal as the concatenation of its fields; \code{self.m.at(a)}
...
it, a receive asserts \code{ready} under the guard, a send drives
