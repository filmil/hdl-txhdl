...
\caption{What the rasteriser drew, read back out of the memory it
shares with the core, while the core was running a program and saying
its line on the serial port two hops away.}
\label{fig:n-soc}
\end{figure}
...
simulation-only \code{Ram}, holding the program's constants at
\code{0x1000} and the framebuffer at \code{0x4000}; the serial port is
the same \code{Uart} the core has always had, with a terminal on its
lines.
...
else, the framebuffer included.}{lst:n-map}

Each corner is the same three pieces: the client, an \code{AxiHost} or
an \code{AxiPer}, and the bridge. Nothing was added to the core or to
the rasteriser to put them here. The rasteriser gained one type
parameter, the framebuffer's base, which was zero and written into its
address arithmetic; on a shared memory it has to be told where its
pixels go.
...
\begin{table}[htbp]
\caption{The same work, on a direct link and across the lattice.}
\label{tab:n-cost}
...
\toprule
 & link & lattice \\
\midrule
The core says its line & 1\,042 & 1\,229 \\
The GPU draws the scene & 12\,656 & 29\,081 \\
\bottomrule
\end{tabular}
\end{table}

The core pays eighteen per cent and the GPU pays a little over twice,
and the difference between them is the difference between latency and
throughput. The core has one load in flight at a time and waits for
each answer, so what it pays is the hops, twice, on the loads it
makes and on nothing else. The rasteriser writes four thousand and
ninety-six pixels and never waits for any of them, so what it pays is
the network's rate: one packet through a node per cycle, and a write
is a packet each way across two hops.

That is the number to improve, and improving it is arbitration and
...
The two overlap for the first twelve hundred cycles, which is as long
as the core runs. A longer program would contend for longer, and the
fixed priority in a switch, which serves the four links before the
exit, is the thing to watch when it does.
