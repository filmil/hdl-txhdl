...
\begin{itemize}
\item A write of one beat is one packet, so it needs nothing kept
between cycles. What the bridge does hold is its refusal of a longer
one: \code{eat}, whether the beats of a refused burst are still
arriving; \code{owe}, whether such a burst is waiting to be answered;
and \code{bad}, the identifier to answer it under.
...
\code{last} high.
\item A write whose \code{len} is not zero is refused instead. The
bridge takes its address phase, sends no packet for it, and takes and
drops every beat that follows until the one marked \code{last}. Then
it answers \code{b} with \code{SLVERR} under the identifier it kept,
in the first cycle \code{b} is ready and the network is not answering.
Issue 280 says why the refusal is there: before it, such a burst left
as one packet marked \code{last}, the peripheral waited for a beat
that had stayed behind, and the host waited for ever.
...
\begin{itemize}
\item Five tests in \code{lib/parts/src/bus/noc.rs}, in
...
\item \code{a\_write\_of\_two\_beats\_is\_refused\_rather\_than\_carried}
asks for two words at once and finds \code{SLVERR} and a memory
nobody wrote. Before the refusal it did not fail an assertion about
the data: it hung.
\item \code{a\_short\_write\_after\_a\_refused\_one\_still\_works}
sends one beat after that burst and finds it served, so the dropped
beats left the channel clean.
...
reassembly at the peripheral, and the bridge has neither. Such a burst
is refused with \code{SLVERR} rather than carried, which is issue 280;
carrying it is issue 125, and that work takes the refusal out again. The map has
