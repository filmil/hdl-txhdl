...
\code{ans} and \code{rb}. The input \code{rst} clears the count and
nothing else.
...
Offset & Word & Access \\
\midrule
\code{0x0000} & \code{msip}, bit 0 & Read and write. Bit 0 raises
the software interrupt; the other 31 bits of a write are dropped and
read back as zero. \\
\code{0x4000} & \code{mtimecmp}, bits 31 to 0 & Read and write. \\
\code{0x4004} & \code{mtimecmp}, bits 63 to 32 & Read and write. \\
\code{0xbff8} & \code{mtime}, bits 31 to 0 & Read and write. \\
\code{0xbffc} & \code{mtime}, bits 63 to 32 & Read and write. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
\item \code{//cpu/vreteno:lockstep} hands \code{pending} and
...
\item \code{//cpu/vreteno:pair} and \code{//cpu/vreteno:board\_test}
