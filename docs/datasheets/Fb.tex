...
% covers: Fb
\datasheet{Fb}{The GPU's framebuffer: a memory of one word per pixel behind an AXI peripheral end.}

\dsfacts{\code{gpu/src/fb.rs}}{\code{gpu::fb::Fb}}%
{\code{//docs:gpu}, \code{//docs:axi}}

\subsection*{Function}

\code{Fb} is \code{N} words of memory, \code{px}, behind the
peripheral end of an AXI link. A pixel is one word. The rasteriser
writes pixels into it, and in the GPU's runs the same memory also
...
no word a meaning. In \code{//gpu:trace} and the tests of
\code{gpu/src/sim.rs}, with \code{N} of 1024:
...
\begin{itemize}
\item \code{//gpu:gpu\_test} renders every scene on the rasteriser
through the link into \code{Fb}, reads the pixels back and compares
each with the model in \code{gpu/src/model.rs}. The tests are
...
\code{//gpu:trace} under nvc and Verilator:
\code{//docs:gpu\_sim\_fb\_fb\_tb\_test} and
\code{//docs:gpu\_vsim\_fb\_test}.
\item \code{//gpu:trace} and \code{//gpu:demo} assert that the
framebuffer equals the model's picture. The build runs
\code{//gpu:demo} to draw the picture in \code{//docs:gpu}.
\end{itemize}

\subsection*{Used by}

\code{gpu::sim::run} and \code{gpu::sim::run\_list}, and so the GPU's
tests, \code{//gpu:trace} and \code{//gpu:demo}. The system's
...
strobe. It holds one write at a time. \code{//docs:gpu} states that
