...
\code{rxb}, which \code{data} reads. The master then drives the
answer bit 4 asked for.
...
The register the byte lands in is \code{rxb} rather than \code{data},
because the VHDL testbench generator names a register's alias
\code{r\_<name>}, which would collide with the \code{r} port's
\code{r\_data} and make the co-simulation compare the wrong signal.
That is issue 211, and the name is the workaround.
