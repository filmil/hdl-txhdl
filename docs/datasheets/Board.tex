...
holding the rest. It holds the core and its tracker, a second host's
pins, an arbiter that puts the two onto one link, a router of seven,
and the peripherals behind its seven ports. The second host is Vivado's
JTAG-to-AXI master on the board's top (issue 241), whose pins arrive
under \code{jtag\_} and are joined by an \code{AxiPins}; the arbiter
is an \code{Arbiter2}, so the peripheral side of the bus carries four
bits of identifier, two for a host's own and two for the port. The data memory and the
...
holds AMD's MIG controller as a foreign module, which makes the
design's clock from the board's. The interrupt
controller, \code{Plic2}, and the debug module, \code{Dm}, each sit
...
board the interrupt controller's second source never asks. The core and the JTAG master are the hosts, the core on port 0 of the
arbiter.
