...
Offset & Word & Access \\
\midrule
\code{0x00} & \code{ctrl} & Read and write. Bit 0, \code{CTRL\_RUN}, is
whether the buffer takes what it is offered. Bit 1,
\code{CTRL\_FREEZE}, is whether \code{halt} stops it. \\
\code{0x04} & \code{count} & Read only. How many entries are held, up
to \code{N}. A write is taken and does nothing. \\
\code{0x08} & \code{cursor} & Read and write. Which held entry the four
words below return, counting from the oldest. \\
\code{0x0c} & & Reads as zero. \\
\code{0x10} & \code{word0} & Read only. Bits 31 to 0 of that entry. \\
\code{0x14} & \code{word1} & Read only. Bits 63 to 32. \\
\code{0x18} & \code{word2} & Read only. Bits 95 to 64. \\
\code{0x1c} & \code{word3} & Read only. Bits 127 to 96. \\
\bottomrule
\end{tabular}
\end{center}
...
\item A read is answered on \code{r} in the cycle its address beat is
...
cycle, and room on \code{b}; it is answered \code{OKAY} in that cycle.
...
\code{fewer\_than\_a\_ring\_are\_held\_oldest\_first} offers three and
...
\code{LiteBridge1} on an AXI4 link at \code{0x1000}, offers twelve
