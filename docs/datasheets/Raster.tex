...
ready, then for each instruction fetch its six words and walk its box a
pixel a turn, and when the list is drawn wait for ever. The lowering
...
\begin{itemize}
\item The front end issues a read of the count at \code{CTRL} once
\code{issue} has room, and waits for its beat. The count's low eight
bits go into \code{left}. A count of zero means poll again.
\item For each of \code{left} instructions it waits a cycle, for the
instruction's index to read back, then reads the six words one at a
time: each read is issued once \code{issue} has room, and the front end
waits for its beat before the next. Each word's fields are latched as
the word lands. A kind field of 3 is taken as a triangle.
...
and \code{wbeat} have room, which adds one to \code{issued}. The walk
steps when the pixel needs no write or its write went out, so
backpressure holds the walk.
\item A pixel's write is a burst of one beat, size 2, \code{INCR},
with the colour zero-extended to 32 bits, strobe \code{0xf} and
\code{last} set.
\item When the last instruction's box is walked, \code{finished} is
set. \code{inflight} is a wire, \code{issued} less \code{answered},
...
The code adds these. It has no reset port, and it waits for ever once the list
is drawn, so it draws one list. \code{left} is eight bits. The pixel's offset from
