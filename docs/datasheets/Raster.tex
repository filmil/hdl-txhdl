...
% covers: Raster
\datasheet{Raster}{The GPU's rasteriser: it reads a display list over AXI and writes one pixel a cycle.}

\dsfacts{\code{gpu/src/raster.rs}}{\code{gpu::raster::Raster}}%
{\code{//docs:gpu}, \code{//docs:noc}}
...
rasteriser of \code{//gpu:trace}: a sixteen by sixteen screen at
...
An instruction's words are in the format \code{gpu/src/dl.rs} states.
...
\begin{itemize}
\item \code{//gpu:gpu\_test} renders scenes on the rasteriser through
an \code{AxiHost}, an \code{AxiPer} and \code{Fb}, and compares every
pixel with the model of \code{gpu/src/model.rs}:
...
0x400, 0x600>} against the trace of \code{//gpu:trace} under nvc and
Verilator: \code{//docs:gpu\_sim\_raster\_raster\_tb\_test} and
\code{//docs:gpu\_vsim\_raster\_test}.
...
tests, \code{//gpu:trace} and \code{//gpu:demo}. The system in
\code{soc}, where the rasteriser is the host at corner 1,0 of the
lattice.

\subsection*{Limits}

\code{//docs:gpu} lists them. It writes one pixel per cycle at most. It
