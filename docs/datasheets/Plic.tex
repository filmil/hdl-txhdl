...
\datasheet{Plic}{The platform-level interrupt controller: one to thirty-one sources and one target, at the RISC-V PLIC's offsets.}
...
source to serve. Each source \textit{i}, numbered from 1, is an input line,
\code{srcs\_}\textit{i}\code{-1}, of the array of ports \code{srcs}. The output \code{irq} is the
...
\code{LiteBridge1}, which sends it only its own range.
