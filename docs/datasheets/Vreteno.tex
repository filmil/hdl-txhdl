...
traps with cause 2, and \code{mtval} takes the word. A halfword that
is no RV32C instruction expands to itself, which no thirty-two bit
instruction is, so it traps with cause 2 and \code{mtval} takes the
halfword. Every other trap writes zero to \code{mtval}.
...
\code{compressed\_instructions}, and \code{random\_programs}, which
...
bits 1 and 0. The core has machine mode only, the eight control registers
