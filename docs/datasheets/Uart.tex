...
The map is declared as \code{serial}. A write to \code{tx} starts a
frame if the port is idle, and otherwise the byte is dropped. A read of
\code{rx} takes the oldest byte out of the buffer, which is what its
access says. Writes to \code{status} and \code{rx} are ignored, and
