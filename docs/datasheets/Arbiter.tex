...
\code{Board}, as \code{arb}, an \code{Arbiter2<32, 32, 4, 2, 4, 0>} in
front of its router: the core's tracker on port 0 and the JTAG
master's pins on port 1 (issue 241), with a peripheral side of four
bits, room for four hosts. Issue 151's direct memory access engine is
the third host, on the same arbiter.
