...
answers must not be able to stop the bus. The tables use 40, as
\code{ex\_remote} does. \\
\bottomrule
\end{tabular}
\end{center}
...
\item \code{//lib/examples:ex\_remote} runs it against a model in the
same program, and the build simulates the netlist against that run
under nvc and Verilator:
\code{//docs:remote\_sim\_remote\_tb\_test} and
\code{//docs:remote\_vsim\_test}.
\end{itemize}

\subsection*{Used by}

\code{ex\_remote} today. Issue 297 is what it is for: a peripheral and
its software co-developed, with the far end eventually across a wire.
...
and an \code{Answer} back, is somebody else's, and until that exists
the far end is a model in the same program. The timeout is counted in
cycles rather than in time, so a design that slows its clock waits
