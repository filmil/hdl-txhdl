...
Offset & Word & Access \\
\midrule
\code{0x00} & \code{dout} & Read and write. Driven on a pin whose
direction is out. \\
...
\code{dout} is named for VHDL, which reserves \code{out}; the lowering
neither escapes nor rejects a reserved word, which is issue 77.
...
\item A read is answered on \code{r} in the cycle it is taken, with
\code{OKAY}. A write is taken when its address phase, its beat and
room on \code{b} are all there, and is answered \code{OKAY} in that
cycle.
