...
\subsection{Figures are ASCII}

Every figure here is a listing, not a drawing. That is a deliberate choice
and it follows an existing project rule: a design document illustrates a
module's architecture with ASCII art. It also removes the drawing packages
from the dependency list, and removes the class of defect that produces no
warning and no error, where a document compiles clean and a figure is still
wrong because an arrowhead is buried in a box border.
