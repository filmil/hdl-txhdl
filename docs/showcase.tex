...
Together they are 47\,606 lines of Rust: 12\,811 of runtime
and macros, 13\,220 of parts, 8\,525 in sixty-six examples, 10\,125 of
Vreteno with its board, and 2\,925 of Razboj, DDR3, Ethernet, HDMI,
PCIe and the SoC.
...
The examples' own units & examples & 35, all but one \\
\bottomrule
\end{tabular}
\end{table}
...
The board & core, 6 devices, DDR3 & 3 runs, Vivado \\
...
\begin{tikzpicture}[x=1cm, y=1cm, every node/.style={font=\scriptsize}]
  ...
  \node[box] (rt) at (3.7,0) {router};
  \node[box] (rom) at (5.9,2.3) {boot\\memory};
  \node[box] (dmem) at (5.9,1.4) {data\\memory};
  \node[box] (uart) at (5.9,0.5) {serial};
  \node[box] (ddr) at (5.9,-0.35) {DDR3\\peripheral};
  \node[boxf] (uber) at (8.1,-0.35) {UberDDR3\\foreign};
  \node[box] (tim) at (5.9,-1.35) {timer};
  \node[box] (plic) at (5.9,-2.2) {interrupt\\controller};
...
\end{tikzpicture}}
\caption{The board: the core, its tracker, the router that decides by
address, and the six peripherals behind it. The DDR3 controller is a
foreign module, wired rather than written; the serial port reaches two
pins, and the interrupt controller and the timer reach the core's two
lines.}
\label{fig:s-board}
\end{figure}
...
its ports. \code{bazel test //...} runs two hundred and ten
targets, of
which a hundred and twenty-six are netlists under both simulators; the rest are the
core's model and
programs, the board's own run, Razboj, the parts, the SoC's run, and
the tools. Four more are tagged manual: the board and its DDR3 under
Vivado's simulator, the AXI link's fuzzing over many seeds, which the
nightly run does, and the check that every component has a datasheet,
which a sweep of the documents runs. Every claim about what
