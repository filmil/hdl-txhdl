...
Together they are 38\,119 lines of Rust: 11\,473 of runtime
and macros, 10\,407 of parts, 7\,124 in fifty-seven examples, 6\,200 of
Vreteno with its board, and 2\,915 of Razboj, DDR3, Ethernet, HDMI,
PCIe and the SoC.
...
its ports. \code{bazel test //...} runs a hundred and twenty targets, of
which a hundred and four are netlists under both simulators; the rest are the
core's model and
...
The lowering names a wire as its \code{let} is written, so a
\code{let} Rust accepts can still be refused, or renamed, on its way
to the netlist. The core cannot load a program it
was not built with, because its instruction memory is not on the bus.
Razboj has no depth buffer, no interpolation and no texture. Each is an issue
