...
Together they are 67\,357 lines of Rust: 17\,189 of runtime
and macros, 18\,892 of parts, 14\,343 in a hundred and three examples, 13\,878 of
...
The examples' own units & examples & 58, in 48 files, all 48 \\
\bottomrule
\end{tabular}
\end{table}
...
its ports. \code{bazel test //...} runs three hundred and forty-eight
targets, of which two hundred and two are netlists under both
simulators; the rest are the core's model and programs, the board's
own run, Razboj, the parts, the SoC's run, ten proofs, the width of
every waveform, and the tools. Seven more are tagged manual: the
board and its DDR3 under
...
datasheet, which a sweep of the documents runs.

A run checks the inputs it was given; a proof checks all of them. A
unit states what must hold with \code{check!}, what it assumes of its
inputs with \code{assume!} and what it should reach with
\code{cover!}, and \code{formal\_test} hands the netlist's assertions
to SymbiYosys on the pinned Yosys: \code{abc pdr} proves a digit's
checks for every input and reachable state, two broken copies must
fail, z3 finds the cycle a broken check fails in, and a cover run
finds the path to the cover point. A unit with its own reset is
proved to check only out of it (issue 633). Those are the ten proofs,
a second each, and \code{//docs:prove} is their document.

Every claim about what
