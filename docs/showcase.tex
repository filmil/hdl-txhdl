...
Together they are 52\,245 lines of Rust: 12\,811 of runtime
and macros, 14\,338 of parts, 9\,467 in seventy-one examples, 12\,681 of
Vreteno with its board, and 2\,948 of Razboj, DDR3, Ethernet, HDMI,
PCIe and the SoC.
...
\begin{tikzpicture}[x=1cm, y=1cm, every node/.style={font=\scriptsize}]
  ...
  \node[boxf] (uber) at (9.4,-0.35) {UberDDR3\\foreign};
  ...
  \draw[fl] (ddr) -- (uber);
  \draw[fl] (uber.east) -- ++(0.75,0) node[lbl, right] {pads};
...
its ports. \code{bazel test //...} runs two hundred and thirty-one
targets, of
which a hundred and thirty-six are netlists under both simulators; the rest are the
core's model and
programs, the board's own run, Razboj, the parts, the SoC's run, and
the tools. Five more are tagged manual: the board and its DDR3 under
Vivado's simulator, the PCIe endpoint and its core under the same, the
AXI link's fuzzing over many seeds, which the nightly run does, and the
check that every component has a datasheet, which a sweep of the
documents runs. Every claim about what
