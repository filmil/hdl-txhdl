...
Tables~\ref{tab:documents} and~\ref{tab:records} name every
document, in the order the combined PDF binds them, with the target
that builds each. The first holds the documents on the language and
what was built in it; the second, the ones that summarise and record.
...
TxHDL by Example
  ...
  & \code{//docs:examples} \\
\addlinespace
Vreteno: An RV32IMC Core in TxHDL
...
Formal Verification in TxHDL
  & The tutorial on proving what a unit states about itself: the
    digit of \code{ex\_formal} proved for every input with SymbiYosys
    on the pinned Yosys, a broken copy failing with the counterexample
    drawn, and the cover point reached, every log the build's. For
    somebody who wants a check to hold for every input rather than
    for one run.
  & \code{//docs:prove} \\
\addlinespace
...
\addlinespace
Ethernet in TxHDL
  & A part on the bus: a MAC in two halves that store whole frames, a
    CRC-32 written as its eight steps, an AXI-Lite peripheral of three
    words,
    a run on a loop of wire with one frame corrupted, and the
    hand-written echo design for the AX7A200B's PHY.
  & \code{//docs:eth} \\
\addlinespace
HDMI in TxHDL
  & A peripheral for an encoder chip: the raster, a framebuffer a host
    paints over AXI-Lite, the I2C master that configures the chip, a
    run that checks every pixel on the screen, and the hand-written
    demonstration for the AX7A200B's SiI9134.
  & \code{//docs:hdmi} \\
\addlinespace
The TxHDL Flagship
  & The one bitstream that holds all of it: the core with its memory,
    the Ethernet port and the HDMI output in one part, three clock
    domains kept apart, the loader in the boot memory, and the whole
    written to the board's flash.
  & \code{//docs:flagship} \\
\addlinespace
PCIe in TxHDL
  & TxHDL behind an endpoint the build generates: the part that joins
    an AXI master's pins to the link, the registers a host reaches in
    BAR1, and the AX7A200B's design, taken to a bitstream.
  & \code{//docs:pcie} \\
\addlinespace
TxHDL Datasheets
  & A datasheet per component: what it does, its parameters and
    register map, its ports, state and children as its netlist has
    them, how it behaves, how it is checked, and where it is used.
...
\begin{table}[t]
\caption{The documents that summarise and record, and all of them in
one file.}
\label{tab:records}
\centering
\begin{tabular}{@{}p{46mm}p{92mm}l@{}}
...
TxHDL: What Has Been Built
  & A few pages on the whole of it: the rule the language was built on,
