...
% does not do. One column, because it is mostly listings.
...
cycle. The two \code{get}s read the register and the input as the
edge left them. \code{when!} is a predicated drive: \emph{when}
\code{en} holds, \code{n} becomes \code{n + 1} at the end of the
step, and the arrow \code{<=} points into the register to say so.
...
Hardware has no \code{if}. Both arms of a choice exist at once and a
condition selects, which is why the three ways of choosing have their
own names, so that the file reads as the hardware does. \code{when!}
you have seen: a drive under a condition, with an optional
\code{else}. \code{case!} is \code{when!} with many arms, and it is
what a state machine is made of. Listing~\ref{lst:t-case} is a
...
\code{when!} and \code{case!}, and that is the whole vocabulary of
choice.
...
there is room on the other channel, \code{recv} the take, \code{send}
the offer. In the netlist a channel is a \code{valid}, a \code{ready}
...
\code{when!}, whose condition becomes the channel's \code{valid}.
...
\lstinputlisting[linerange={32-48},caption={The counter's testbench: wires, a waveform, the unit running, twelve cycles. From \code{ex\_cheat.rs}.},label={lst:t-main}]{lib/examples/ex_cheat.rs}
...
named by \code{let} is a wire of that name. \code{when!} and
\code{case!} are predicated assignments in the clocked block.
...
are functions of what was read, and drives. Inside it you have the
operators of the value types, \code{slice}, \code{concat},
\code{sext} and \code{zext} for taking words apart and putting them
together, and \code{mul::<M>} for a product at a stated width. What
you do not have inside a lowered loop is what hardware does not
have: \code{if}, a Rust \code{match}, a function call the lowering
does not know, or a loop with a variable count. The macro says so at
compile time, with a message that names the construct.
