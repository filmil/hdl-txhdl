...
tutorial takes you from nothing to that in eight steps, each on a
small example the build runs: a counter, the model of time, the ways
of choosing, channels between units, ports with names, units made of
units on more than one clock, the waveform, and the netlist with its
checks. A ninth step shows how to add an example of your own, which
is how everything here was added.
\end{abstract}
...
the counter's Verilog, which is Step Eight. The \code{rst} among its
ports is the reset, which every module gets without asking; Step Six
says what it does. Fig.~\ref{fig:t-counter}
...
So a process may wait more than once in its loop and take a step a
cycle: an address at one edge, the data at the next. The lowering
makes such a loop a state machine with a state per wait, and a wait
under \code{if} a state the process visits only when the condition
holds. An output set in one state keeps its value until another state
sets it, as a wire in the run does. Step Eight has the rest.
...
these. \code{mux(c, a, b)} is the two-way form. Rust's own forms
work as well: an \code{if} with an \code{else} that yields a value,
and a \code{match}, lower to the same chain of conditions, and a
pattern may be a range, \code{0x30..=0x39}, as may
\code{(lo..=hi).contains(\&x)}. You choose between values with
\code{select!}, \code{mux}, a value \code{if} or a \code{match};
between drives with \code{with!}, \code{when!}, \code{case!} and
\code{if}. That is the whole vocabulary of choice.
...
The units so far had one port on each side. A unit with more could
take a tuple, but a tuple is read by position, so swapping two ports
of the same type compiles and is wrong. Name them instead. A side of
a unit may be any struct of ports: its fields are the ends of
channels and wires, or smaller structs of ports, and each is named
where it is declared, where it is passed and where it is read.
Listing~\ref{lst:t-ports} is a subtractor whose two operands have the
same type and cannot be swapped by accident.
...
\code{Diff} takes \code{DiffIn} and gives \code{DiffOut}. Its
\code{run} takes the input side apart in its signature, as any Rust
function may, and reads the output side by field: \code{out.diff},
\code{out.borrow}. A struct of ports may be generic, as these are
over the width, and it may be declared in another crate:
\code{\#[derive(Ports)]} on it is all the lowering asks. Every
AXI-Lite peripheral in \code{//lib/parts} is handed its link as one
such struct, \code{LitePort}. Such a peripheral says its registers once,
in \code{regmap!}: each register's place, whether a host reads it,
writes it or both, and its bit fields. What \code{run} reads and
writes goes through the map, and the same declaration writes the C
header a driver includes and the datasheet's table, as
\code{ex\_regmap} shows.

A side may also be an array of ports, \code{ins: [Rx<U<8>>; N]},
\code{N} a const parameter. The body goes over it with
\code{for i in 0..N}, which the lowering unrolls for each \code{N},
and the netlist's ports are \code{ins\_0} onward, as \code{ex\_loop}
shows. The AXI arbiter in \code{//lib/parts} is one unit for any
number of hosts this way.

\code{interface!} declares the members of a link once and writes a
struct per \emph{role}, each role saying which way each member goes.
\code{Operands} has two channels: its \code{Source} role sends on
both, its \code{Sink} receives on both, and \code{Operands::new()}
makes the two roles, joined. \code{Top} takes the sink and passes its
members to \code{Diff} in a struct literal, by name, so the order in
which they are written no longer matters.

The netlist flattens each struct into its fields.
Listing~\ref{lst:t-ports-out} is the top of the module the lowering
wrote: each channel is its data, its \code{valid} and its
\code{ready}, under the field's name. Fig.~\ref{fig:t-ports} is the
run.
...
A design bigger than a counter is a unit that holds units. The
children are fields of the parent. Its \code{run} hands each child its
ports and runs them together, with \code{join2} for two and nested
\code{join2}s for more, and the lowering reads that as one module
with an instance of each child. \code{Top} in
Listing~\ref{lst:t-ports} is one already, with a single child.
A parent can pass a child a side it took whole, \code{bus}, even when
the struct is declared in another crate. It makes a whole link
between two children in one line,
\code{let (host, per) = link::<LitePort<32, 32, 4>>()}, rather than a
channel at a time, and it holds a child's input at a constant with
\code{tie(v)} where the input is passed.

A unit's process waits on one clock, so a second clock is the other
reason to have children. A clock is a type: a struct that implements
\code{Clock}, with a name and a period in ticks. \code{DefaultClock}
has a period of two. Listing~\ref{lst:t-twoclock} declares a clock
three times slower, and two counters. One runs on the default clock,
which it never names. The other runs on \code{Slow}, and names it in
its wait, its register and both its ports: \code{Reg<U<8>, Slow>},
\code{In<Bit, Slow>}, \code{Out<U<8>, Slow>}. The parent, \code{Two},
names no clock at all. The module the lowering writes has an input
for each clock, and the build checks each port against the edges of
its own clock. Fig.~\ref{fig:t-twoclock} shows the two counts, one
step of the slow count to three of the fast.
...
Nothing passes between the two counters, and that is deliberate. The
clock is part of a port's type, so a signal on one clock cannot be
handed to a unit that wants another; \code{Crossing<T, A, B>} is the
one thing that turns one into the other. A channel crosses through
\code{ChanCdc} from \code{//lib/parts}: two processes, one on each
clock, a memory between them, and each side keeping the other's Gray
pointer through two flops of its own clock. The example
\code{ex\_cdc} runs one, and the build checks its netlist as it checks
any other.

One port you never write is the reset. Every module the lowering
writes gets an \code{rst} input, as it gets its clocks; while it is
high, every register goes back to what it held before the first edge,
and the channels empty. The counter's Verilog in
Listing~\ref{lst:t-counter-out} has it. A unit that must do something
under reset, rather than just forget, takes \code{rst: In<Bit>} and
reads it, and the netlist joins that port to the same net. The
Vreteno core does this, because it has to fetch from its reset
vector.
...
\code{waveform()} entry of Step Nine.
...
are functions of what was read, and drives. A loop of several waits
is read too, as a state machine with a state per wait, which
\code{ex\_seq} in the examples document shows. A wait under \code{if}
is a state with a way around it, as \code{ex\_frame} shows. Inside it you have
...
\code{mul::<M>} for a product wider than its operands. A value may be wider
than 128 bits: \code{U<256, 2>} keeps its bits in two limbs of 128,
the second parameter the number of limbs. An \code{if}
...
call the lowering does not know, or a loop with a variable count. A
\code{for i in 0..N} has a fixed count, since \code{N} is a const
parameter, and is unrolled.
The macro says so at compile time, with a message that names the
construct.

A function the lowering does know is one that carries
\code{\#[lower]} itself, in the same file: it stays plain Rust for
the simulation and is inlined at every call, so a step can keep its
arithmetic in named pieces, as \code{ex\_fn} does. Names are yours to
choose. A port called \code{next}, \code{begin} or \code{out}, or a
register called \code{signal}, is a word VHDL or SystemVerilog
reserves; the netlist writes it with \code{\_rw} after it, the same
in both and in the generated testbench, so \code{next} is
\code{next\_rw} and the co-simulation still finds it.
...
That comparison says the netlist does what the run did, not that
either is right. For that, a unit states what it knows about itself
where it is written: \code{check!(c, "why")} must hold where it is
reached, \code{assume!} is what the unit takes for granted of its
inputs, and \code{cover!} is what it should be possible to reach. The
run panics on a broken check or assumption and counts each cover
point, and the netlist carries them as assertions, which nvc and
Verilator check as they replay the run and a formal tool can prove
for every input. \code{ex\_formal} states all three.
...
\item Write a section for it in \code{docs/examples\_sections}, with
  a number in its name that sorts it where it belongs; the build
  includes every file there in the order of the names. Add the run
  and the figure to the examples document's \code{data}. Build the
  document and look at the page.
\end{enumerate}
...
The examples document~\cite{examples} has one of these per concept,
in the order of the language, with the runtime's public interface at
the end for looking things up. Three more tutorials pick up where
this one stops: \code{//docs:zero} builds a blinky in an empty
directory against the published repository, \code{//docs:axi} is the
AXI bus from one register to a router of peripherals, and
\code{//docs:station} is the reservation station read cycle by
cycle. The embedding
...
is what the eight steps add up to.
