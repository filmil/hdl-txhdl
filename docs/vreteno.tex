...
bit, a bit's last cycle and middle and a bit taken in, are functions
under \code{\#[lower]}, inlined where the step calls them, so the step
is the two sides' drives; the examples document~\cite{examples}
states the form. Its three words are declared once, with
\code{regmap!}, the third as a register the read takes, and the
macro writes the word a load reads, which word a write goes to,
when a read takes a byte, and the offsets the Zephyr driver is held
to (issue 669). The run and the lockstep
