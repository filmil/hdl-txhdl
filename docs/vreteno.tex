...
whose six sixteenths are the serial port, the pulse width modulator,
a slot for a peripheral on a clock of its own, the remote
peripheral whose transactions leave the board as Ethernet frames, the
Ethernet port's registers, and the entropy source, the
board's DDR3 memory from \code{0x4000\_0000}, a
gigabyte behind the bridge from AXI to Wishbone and AMD's MIG
controller, which the build generates, and the interrupt controller at \code{0x0c00\_0000}, whose
...
being what its simulation variant carries, and the core says
...
met: the worst setup slack is 0.547~ns, the worst hold slack
0.039~ns, over 28\,309 endpoints on the clocks the controller's
...
unconstrained. The design takes 10\,964 LUTs, 9\,850 flip-flops, four
block RAM tiles, four DSP blocks, 81 pins, the controller's PLL and
MMCM, and its 64 output and 32 input serialisers with 32 delays. The memory's pins carry no input or output delay, as in
...
UberDDR3 as the controller; the board carries AMD's MIG since issue
188, GPL-3.0 having no place in a design this tree claims as
Apache-2.0, and the run with MIG is owed the same way.
