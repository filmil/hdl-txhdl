...
byte in the port's third word, where a load takes it. The port's
interrupt line is high from the landing to the load; on the board
it is the core's external interrupt, and the runs join it to the
line they drive. Its second word is the status, bit 0 busy while a
frame is going out and bit 1 a byte received and not yet read. A
byte written while busy is dropped, and a byte received before the
last was read replaces it, so a program polls the status.
...
the byte lands, the ready bit rises and with it the interrupt line.
...
shifting in, and the ready bit and the interrupt as it lands.}
\label{fig:v-in}
\end{figure}
...
takes; the serial port holds one byte received, and a buffer of a
few would let a program type ahead. Each begins in the model, as
