...
same way: \code{bazel run //cpu/vreteno/board/remote:serial} runs a
small program on the far machine, over ssh and under a timeout that
frees the port whatever happens here, which prints what the board
...
LEDs are on the other machine and were not looked at. Holding the
reset puts the halt LED out; releasing it says OK and waits for
three keys again.
