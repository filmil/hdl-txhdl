...
unit of units holding the core and its tracker, a router of four, a
tracker for each peripheral but the serial port, which sits behind its
AXI-Lite bridge, the data memory at \code{0x1000}, the
timer at \code{0x2000}, the serial port at \code{0x3000}, and the
board's DDR3 memory from \code{0x4000\_0000}, a gigabyte behind the
bridge from AXI to Wishbone and UberDDR3's controller. The controller
...
met: the worst setup slack is 0.220~ns, the worst hold slack
0.040~ns, over 17\,708 endpoints on the four clocks Vivado derives
...
takes 7\,661 LUTs, 6\,403 flip-flops, three block RAM tiles, four DSP
