...
The bus has three devices, and a router between the core and them,
...
one is the data memory, two the timer and three the serial port. The
devices' responses merge back into one channel. The router takes a
request only when the channel it goes to has room. That is a receive
under a condition of its own, \code{recv\_if}, whose \code{ready}
is the condition and the offer together. An address that is no
...
waits for each load's answer before it makes another request.
...
\lstinputlisting[caption={\code{dmem.rs}: the data memory, a device
on the bus.},label={lst:v-dmem}]{cpu/vreteno/src/dmem.rs}
...
and are in the board's figures below.}
\label{tab:v-synth}
\centering
\begin{tabular}{@{}lr@{}}
\toprule
...
100~MHz with 1.094~ns of slack, in 2050 LUTs and 1062 flip-flops
for the core, the router, the data memory, the timer, the serial
port with both its sides and its buffer of eight, and the eight
channel buffers together, three block RAM tiles, four DSP blocks and
the MMCM, with no critical warning and no combinational loop, which
the first board top with the units wired port to port had. The
port's buffer is six LUTs of distributed memory. The wait's move
into writeback took the board's slack from 0.285~ns to 0.903, and a
tenth of the LUTs with it; the step's pieces as functions moved it
to 0.688, the port's receive side to 1.074, its buffer to 1.004, and
the memory's move onto the bus to this: placement moves a path of
routing by tenths from run to run.
...
takes, which is paid now; the board's own memory, behind a
controller, could take the data memory's place at its address, the
core none the wiser. Each begins in the model, as
