...
Place and route, and a board. Section~\ref{sec:v-synth} synthesizes
with a clock constraint and no pins; the ruleset places, routes and
programs a flash as well, and the first board is the Artix-7 the part
was chosen for. Timing at ten nanoseconds is the number to learn
first, and the combinational read of the data memory through the
load's extension into the register file write is where it will be
lost if it is.
