...
in execute; and with an interrupt line, taken in place of the
instruction in execute when \code{mie}, \code{mip} and
\code{mstatus} allow. It is checked twice. In lockstep against a reference model of the instruction set
...
\code{mip}, \code{mtval}, the external interrupt's bit and its
cause. The M extension is the
...
with the interrupt's cause, and returns to the same address. Whether
that step takes the interrupt is the caller's word, since the core
decides on the bit as it stood a cycle earlier, and the lockstep test
carries the core's decision across. Only \code{ebreak} halts. The
...
multiplicand or divisor, and two sign bits; and six memories: the register file, the instruction memory,
...
\code{mtvec} first of all, the external interrupt is enabled in
\code{mie} and interrupts in \code{mstatus}, so that the line can
be raised at any point of the run. An \code{ecall} and then the
word of all ones, which is illegal, enter the handler. The handler
reads the cause into \code{x23} and branches on its sign: an
interrupt is cleared in \code{mip}, counted in \code{x8} and
returned from to the word it interrupted; an exception has
...
five in \code{x24} at the end, and one in \code{x8} when the line
was raised once. Between the shifts
...
ones. Interrupts are enabled from the start. The handler placed
after the \code{ebreak} branches on the cause's sign as the
demonstration's does, clearing an interrupt and returning to the
word it interrupted, and returning from an exception to the word
after; \code{x31} is the handler's and the program leaves it alone.
Nothing in it can loop, so every program halts.
...
Every control was then removed. The single-cycle core passed the
...
line is high, as the core does at its edge.
...
an illegal word or an interrupt is in execute the two disagree by
design. The test
...
prints as one line with the count. The interrupt is the jump to the
handler at \code{0xa4} out of the loop, with the cause's top bit
set in \code{x23} and one counted into \code{x8}, and the return
to the instruction it interrupted; the two traps are the later jumps
to the same handler, the first from the \code{ecall} with eleven
read into \code{x23}, the second from the illegal word with two,
each with its bubble, as the returns have theirs. Figure~\ref{fig:v-wave} is the
opening of the same
...
hardware: thirty-one registers, six memories, the instruction
memory holding the program, a wire per \code{let} of the loop, three
temporaries, and one clocked block of a hundred and six lines. The VHDL the run
...
counter and the outputs that follow; the three hundred and
sixty-six ticks of the demonstration exercise every instruction of
...
the thirty-one registers of the source less one, the writeback
...
design meets ten nanoseconds with 0.534~ns of slack, and hold with
0.105~ns. The critical path is the multiplier: 9.109~ns from the
sequencer's registers into the pair, four DSP blocks and a carry
chain of thirteen, of which logic is four fifths, which is what the
start cycle buys; the branch on a register, through the register
file's read and the compare into the instruction memory's address,
is the next path, and was the critical one before the multiplier
took the DSP blocks. With two stages the slack
...
100~MHz with 0.956~ns of slack, on the branch's path, in 1841 LUTs,
448 flip-flops, three block RAM tiles, four DSP blocks and the MMCM,
...
Section~\ref{sec:v-synth}: the multiplier, which a second register
in the middle of its carry chain, one more step of the sequencer,
would take off the path; then the branch on a register, which a
fourth stage, or a register file read a cycle earlier, would
shorten. The lockstep test is the check for any of these, since it
compares architectural state when an instruction retires.

A timer. The interrupt line is there; what raises it on the board
is not. The manual's timer is a pair of memory-mapped registers
outside the core, and its interrupt is a second bit in \code{mie}
and \code{mip}; a device on the data bus is a first step outside
the core's one address space. Each such step begins in the model, is
added to the core under the lockstep test, and is checked as a
netlist by the same trace, as the traps, the M extension and the
interrupt were.
