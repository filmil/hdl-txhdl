...
scales past its examples. This is its fourth form: a three-stage
...
enters the handler, which returns through \code{mret}. It is checked
...
causes it raises, are constants beside them. A unit test checks each
format's encoder against the decoder.
...
halts; the halt itself; the five control registers; and six
memories: the register file, the instruction memory,
...
\code{x23} and five in \code{x24} at the end.
...
both outcomes of every branch condition occur, the six CSR forms on
...
byte and half. The one stall is the bubble after \code{lw}, where
the next instruction adds to what it loaded. The two traps are the
...
hardware: twenty-one registers, six memories, the instruction
memory holding the program, a wire per \code{let} of the loop, three
temporaries, and one clocked block of sixty-nine lines. The VHDL the run
...
counter and the outputs that follow; the hundred and eighty ticks of the
demonstration exercise every instruction of the program through
both, the two traps and their returns included, since a wrong
\code{mepc} or \code{mtvec} is a wrong program counter. The checks that failed on the way, a truth value rendered
...
the twenty-one registers of the source less one, the writeback
...
design meets ten nanoseconds with 0.737~ns of slack, and hold with
0.079~ns. The critical path is 8.732~ns, of which routing is three
quarters, through twelve levels of logic: from the writeback stage's
destination register, through the forwarding compare into the
operand and the branch compare, to the instruction memory's address,
which is a branch on what the instruction before it computed. With
two stages the slack had been 0.263~ns, on a path from a register
through the decode and the ALU into the register file's write. With
three stages and the load forwarded it was 0.361~ns, on a load
followed at once by a jump through what it loaded. With the traps,
whose return and handler addresses widened the fetch's multiplexer,
that path missed by 0.36~ns. That is what the stall of
Section~\ref{sec:v-core} was for: it took the data memory's read
and the load's extension off the forwarding path, and the slack
doubled. The tool had an opinion of its own on the way. Left to
itself it moved three of the four data memory lanes into
distributed RAM to shorten that path, so the constraints file pins
the lanes to block RAM, where the third stage put them.
...
100~MHz with 0.744~ns of slack, in 1348 LUTs, 309 flip-flops, three
...
More clock. The stall took the load off the critical path; what
sets the timing now is stated in Section~\ref{sec:v-synth}, and a
fourth stage, or a register file read a cycle earlier, would let
the clock rise, the lockstep test being the check for either.

The multiplication extension, and the rest of the privileged
architecture: \code{mtval}, the timer and external interrupts, which
need a way in from outside the core. Each begins in the model, is
added to the core under the lockstep test, and is checked as a
netlist by the same trace, as the traps were.
