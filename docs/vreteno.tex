...
100~MHz with 1.074~ns of slack, in 2016 LUTs and 984 flip-flops
for the core, the router, the timer, the serial port with both its
sides and the six buffers together, three block RAM tiles, four DSP
blocks and the MMCM, with no critical warning and no combinational
loop, which the first board top with the units wired port to port
had. The wait's move into writeback took the board's slack from
0.285~ns to 0.903, and a tenth of the LUTs with it; the step's
pieces as functions moved it to 0.688, and the port's receive side,
fifty-seven flip-flops, to this: placement moves a path of routing
by tenths from run to run.
