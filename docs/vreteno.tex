...
in execute. It is checked twice. In lockstep against a reference model of the instruction set
...
prints as one line with the count. The two traps are the
...
thirty-four ticks of the demonstration exercise every instruction of
...
design meets ten nanoseconds with 0.617~ns of slack, and hold with
0.077~ns. The critical path is 8.853~ns, of which routing is three
quarters, through ten levels of logic: from the register file's
read, through the branch compare, to the instruction memory's
address, which is a branch on a register. The multiplier is the next
path: four DSP blocks and a carry chain of thirteen, 8.7~ns of which
logic is six sevenths, from the sequencer's registers into the pair,
which is what the start cycle buys. With two stages the slack
...
100~MHz with 1.184~ns of slack, on the multiplier's path this time,
in 1796 LUTs, 414 flip-flops, three block RAM tiles, four DSP blocks
and the MMCM, with no critical warning.
...
Section~\ref{sec:v-synth}, and a fourth stage, or a register file
read a cycle earlier, would let the clock rise; the multiply could
also lose its start cycle by taking the product from the operands
rather than from the registers, if the timing allows. The lockstep
test is the check for any of these, since it compares architectural
state when an instruction retires.
