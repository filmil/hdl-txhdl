...
unit of units holding the core and its tracker, a router of seven, and
...
behind an AXI-Lite bridge. The page at \code{0x3000} is six
...
board as Ethernet frames, the Ethernet port's registers, and the
entropy source. Both tables are written by \code{//tools/memmap} from
...
The DDR3 memory is a gigabyte behind the bridge from AXI to Wishbone
and AMD's MIG controller, which the build generates. The controller
...
Table~\ref{tab:v-uart}, the Ethernet port's registers in
Table~\ref{tab:v-ethslots}, and the entropy source in
Table~\ref{tab:v-trng}. The page's other peripherals do not declare
theirs yet, and a table joins these when one does.
