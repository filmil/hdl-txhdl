...
its output \code{dbg}; the board ties the inputs low until the
debug module of issue 154 drives them, and the lockstep test drives
them from a plan, Section~\ref{sec:v-lockstep}. The CSR
instructions read and write \code{dcsr} and \code{dpc} from any
mode, where the specification allows them only in debug mode; the
restriction waits for the module, since until then a test sets
\code{step} and \code{ebreakm} by writing them from the program.
...
from nowhere read zero and a program carried on as though it had read
...
is a line raised from the first cycle (issue 419). The line is a wire,
...
pending bit for the external interrupt is set by its line and cleared
only by software, so the crate's dispatcher clears it after the
handler; the controller's line fell a cycle after the claim. \code{//cpu/vreteno:board\_test} runs the board's design on it,
...
moved past the end of the run and read back, since the store is
posted and the line drops only when it lands, and either is counted in \code{x8}
...
entry whose cause is the request. A program that sets
\code{dcsr.step}, halted once and resumed as often as it asks, enters
again after every instruction with the step as the cause until the
instruction that clears the bit has run, and then runs to its halt
with every step counted in a register. And a program that sets
\code{dcsr.ebreakm} enters at its \code{ebreak}, with the breakpoint
as the cause and the instruction's address in \code{dpc}, and is
left there, held. The plan with no resume left ends the run in the
...
\code{0x1000}, the timer at \code{0x2000}, the page at \code{0x3000}
whose four sixteenths are the serial port, the pulse width modulator,
a slot for a peripheral on a clock of its own, and the remote
peripheral whose transactions leave the board as Ethernet frames, the
board's DDR3 memory from \code{0x4000\_0000}, a
...
11\,334 registers and 19 block RAM tiles, against the board's 8\,100,
6\,959 and four, and meets its timing with 0.6~ns to spare.
...
names and the master's pins under \code{jtag\_}, joined inside by an
\code{AxiPins} to the link's channels. And the router has an arbiter in
front of it, \code{Arbiter2}, with the core's tracker on one port and
the pins on the other. An arbiter puts the port number above a host's
own identifier, so the peripheral side of the bus carries four bits
where it carried two: two for a host's own and two for the port, which
is room for four hosts before anything widens again, and a direct
memory access engine is the next (issue~\#151). Every peripheral's
...
registers, against the board's 8\,100 and 6\,959, and meets its
...
carries the halt and resume requests and \code{dmactive};
...
is no program buffer and no system bus access: memory is reached
through the bus the module sits on.
...
board, \code{vreteno\_board\_dm\_probe} does the same from the hardware
manager against whatever program runs.
...
where the linker script puts it; the data memory carries only what a
...
nothing sensible to resume. The core's pending bit for the external
interrupt is cleared here, after the handler, which is the one thing
about this core a portable dispatcher would not know.
