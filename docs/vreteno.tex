...
unit would use \code{case!} over enums, and functions where it would
use operators in arms. Its memories lower already: a \code{Mem} is an
array in the netlist, written through \code{at} and read by index,
which the examples document~\cite{examples} shows checked under nvc.
The decode and the selection are what remain, and each has a
precedent there. When the core lowers, the same trace that checks
the model checks the netlist.
