...
the router answers \code{DecErr} itself. Each device is behind an
\code{AxiPer}, so what a device sees is a whole request and, for a
write, its beat; none of them decodes an address any more, because
the router has already done it. Nothing in this section is the
...
an AXI peripheral.},label={lst:v-uart}]{cpu/vreteno/src/uart.rs}
