...
file and retires, with the one hazard closed by forwarding and one
bubble per taken branch or jump, written in the subset the lowering
reads, so that the same file simulates and is a netlist. It is checked twice. In lockstep against
...
lane and the width to take from it, and whether it halts; the halt
itself; and six memories: the register file, the instruction memory,
...
instruction two behind reads what it wrote. No stall is ever needed.
Register zero reads as zero before that check, so a writeback to
nowhere forwards nothing.
...
not, on a taken branch or a jump, that word is the wrong one: the
instruction register is marked empty, the fetch counter takes the
target, and the next cycle executes nothing, which is the one-cycle
penalty a machine pays for resolving branches in its second stage.
Reset empties the instruction register and restarts the fetch at
zero; a halting instruction stops the fetch and parks its counter on
itself, as the model's counter parks, and sets the halt a cycle
later when it retires, so the two agree there as well.
...
and the four lanes' writes under their enables, the writeback
registers under execute's run, and the fetch's state by reset, halt,
redirect or advance, in that priority. Register zero
...
hardware: fifteen registers, six memories, the instruction memory
holding the program, a wire per \code{let} of the loop, three
temporaries, and one clocked block of fifty lines. The VHDL the run
...
the fifteen registers of the source less one, the writeback stage's
program counter, which only the lockstep test reads and synthesis
removed as unused, with a warning that says so.
...
design meets ten nanoseconds with 0.361~ns of slack, and hold with
0.097~ns. The critical path is 9.074~ns, of which routing is five
eighths, through ten levels of logic: from a data memory lane's read
port, through the forwarding path into the ALU, to the instruction
memory's address, which is a load followed at once by a jump through
what it loaded. With two stages the slack had been 0.263~ns on a
path from a register through the decode and the ALU into the
register file's write, and the third stage took the data memory's
read off that path and put the load's data on the forwarding path
instead; the core runs at 100~MHz on this part either way, with a
third of a nanosecond to spare now.
...
100~MHz with 0.285~ns of slack, in 977 LUTs, 178 flip-flops, three
...
More clock. The path that sets the timing now is a load followed at
once by a jump through what it loaded; a stall on that one case, or
a fourth stage, would let the clock rise, and the lockstep test is
the check for either.
