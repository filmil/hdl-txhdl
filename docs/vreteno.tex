...
multiply and divide as one sequencer of thirty-two steps, which
stalls the instruction in execute. It is checked twice. In lockstep against a reference model of the instruction set
...
causes it raises, are constants beside them. The M extension is the
...
counter, the registers, a data memory, the five control registers,
...
model does not have is an illegal word, and traps. Only
\code{ebreak} halts. The M extension is Rust's arithmetic on the
...
halts; the halt itself; the five control registers; the sequencer's
...
Thirty-two steps follow, one a cycle. A multiply shifts the pair
right and adds the multiplicand into the high half when the low bit
was set, which leaves the 64-bit product in the pair; a division
shifts the pair left, subtracts the divisor from the high half when
it fits, and shifts the fit in as the quotient bit, which leaves the
quotient in the low half and the remainder in the high. On the
thirty-third cycle the instruction runs: the result is the pair with
the signs put back, the product or quotient negated when the signs
...
ordinary writeback. One set of registers serves both, and the
hardware is an adder, a subtractor, a compare and some multiplexers,
which is small and slow: thirty-three cycles for any of the eight
instructions. A multiplier in the part's DSP blocks would be one
cycle, and is the next step for the clock and the throughput both;
the sequencer is the form that costs nothing on the critical path.
...
trap or a return, the writeback registers under execute's run, and
...
\code{mtvec}; an \code{ecall} and then the word zero, which is
illegal, enter the handler. The handler reads \code{mepc}, steps it
past the word that trapped, writes it back, reads the cause into
\code{x23} and returns. The program then writes \code{mscratch} by
immediate and reads it back into \code{x24}, so that two is in
\code{x23} and five in \code{x24} at the end. Between the shifts
...
\code{mscratch}, \code{mcause} and \code{mepc}, and now and then
an \code{ecall} or an illegal word, which the handler placed after
the \code{ebreak} returns from to the word after; \code{x31} is
the handler's and the program leaves it alone. Nothing in it can
...
when the stage is empty, the thirty-one registers, the five control
...
\code{mret}, on \code{mstatus}. Every control was then removed.
The single-cycle core passed the first; the pipeline passes them
all.
...
So in the one cycle in which a CSR instruction, a call, a return or
an illegal word is in execute the two disagree by design. The test
...
where the next instruction adds to what it loaded, and each multiply
or divide is a run of thirty-three, which the run prints as one line
with the count. The two traps are the
jumps to the handler at
\code{0x7c}, the first from the \code{ecall} with eleven read into
\code{x23}, the second from the illegal word with two, each with its
bubble, as the returns have theirs. Figure~\ref{fig:v-wave} is the
opening of the same
...
hardware: twenty-eight registers, six memories, the instruction
memory holding the program, a wire per \code{let} of the loop, three
temporaries, and one clocked block of ninety-one lines. The VHDL the run
...
counter and the outputs that follow; the five hundred and twenty
ticks of the demonstration exercise every instruction of the program
through both, the two traps and their returns included, since a
wrong \code{mepc} or \code{mtvec} is a wrong program counter, and
every register of the sequencer at every step of five multiplies and
divides, since the whole unit is traced. The checks that failed on the way, a truth value rendered
...
the twenty-eight registers of the source less one, the writeback
...
design meets ten nanoseconds with 1.013~ns of slack, and hold with
0.110~ns. The critical path is 8.456~ns, of which routing is three
quarters, through nine levels of logic: from the register file's
read, through the branch compare, to the instruction memory's
address, which is a branch on a register. With two stages the slack
...
and its steps are a cycle each. The tool had an opinion of its own
...
100~MHz with 1.151~ns of slack, in 1787 LUTs, 415 flip-flops, three
block RAM tiles and the MMCM, with no critical warning.
...
More clock, and more throughput. What sets the timing now is stated
in Section~\ref{sec:v-synth}, and a fourth stage, or a register file
read a cycle earlier, would let the clock rise; a multiplier in the
DSP blocks, pipelined over two cycles, would turn thirty-three cycles
into two. The lockstep test is the check for any of these, since it
compares architectural state when an instruction retires.

The rest of the privileged architecture: \code{mtval}, the timer and
external interrupts, which need a way in from outside the core. Each
begins in the model, is added to the core under the lockstep test,
and is checked as a netlist by the same trace, as the traps and the M
extension were.
