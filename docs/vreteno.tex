...
same unit lowered twice. The port's pieces, whether an address is
its own, the frame a byte goes out as, the frame after a bit, a
bit's last cycle and the word a load reads, are functions under
\code{\#[lower]}, inlined where the step calls them, so the step is
short and reads as what it does; the examples
document~\cite{examples} states the form. The run decodes its own
line as a terminal would and prints what the demonstration said.
...
100~MHz with 0.688~ns of slack, in 1994 LUTs and 927 flip-flops
...
first board top with the units wired port to port had. The wait's
move into writeback took the board's slack from 0.285~ns to 0.903,
and a tenth of the LUTs with it; the step's pieces as functions
moved it by two tenths, as the core's.
