...
\code{mstatus} allow; and with a timer, four words in the address
map beside the memory, whose interrupt is the second. It is checked
twice. In lockstep against a reference model of the instruction set
...
pending bit is the count having reached the compare, which is read
in \code{mip} but not stored there, and its interrupt comes after
the line's. The model has no clock, so the count is not its own: the
caller sets \code{mtime} before each step to what the core's was
when the instruction executed, a store to the count is left to the
core, and the compare is the model's state. Which interrupt that
step takes, if any, is the caller's word too, since the core decides
on the bits and the count as they stood a cycle earlier, and the
...
multiplicand or divisor, and two sign bits; the timer's count and
compare, and the word a load read from the timer with whether it
did; and six memories: the register file, the instruction memory,
...
The timer is the first thing in the address map that is not memory.
Its four words sit at \code{TIMER\_BASE}, a load or store whose
address falls in those sixteen bytes goes to it rather than to the
data memory, and a store writes the lanes it covers into the word,
as a store to memory does. The count runs from the reset, one a
cycle; the compare is what software sets; the count having reached
the compare is the timer's pending bit, read in \code{mip} and
taken as the line's interrupt is, after it. A load from the timer
takes its word into a register of its own, beside the one the data
memory's read lands in, and the writeback stage chooses between them:
the memory's register stays the block RAM's own that way, with
nothing in front of it, and the multiplexer sits on the writeback's
path, which the stall took off the critical one. The timer sits in
the core for now; putting it on a bus of its own is the step after.
...
line is high, as the core does at its edge. The timer's count goes
across the same way: the test reads the core's count while the
instruction is in execute and hands it to the model with the
instruction, so a load of the count agrees and the timer's interrupt
is decided on the same number in both.
...
hardware: thirty-five registers, six memories, the instruction
memory holding the program, a wire per \code{let} of the loop, three
temporaries, and one clocked block of a hundred and twenty-one
lines. The VHDL the run
...
fifty-two ticks of the demonstration exercise every instruction of
...
multiplies and two divides, since the whole unit is traced. The checks that failed on the way, a truth value rendered
...
the thirty-five registers of the source less one, the writeback
...
design meets ten nanoseconds with 0.719~ns of slack, and hold with
0.135~ns. The critical path is 8.870~ns, of which routing is two
thirds, through seventeen levels of logic: from the writeback
stage's destination register, through the forwarding compare into
the operand, the address add and the timer's decode, to the compare
register's enable, which is a store to the timer of what the
instruction before it computed. The multiplier, four DSP blocks and
a carry chain from the sequencer's registers into the pair, and the
branch on a register through the register file's read and the
compare into the instruction memory's address, are the next two
paths, and each was the critical one in an earlier form. With two stages the slack
...
the interrupt line is tied low, since nothing on the board raises it
yet.
...
100~MHz with 0.621~ns of slack, on the same path, in 2039 LUTs, 613
flip-flops, three block RAM tiles, four DSP blocks and the MMCM,
with no critical warning.
...
Section~\ref{sec:v-synth}: a store to the timer of a forwarded
value, which a bus with a registered address would take off the
path, and which is one more reason for the bus; then the
multiplier, which a second register in the middle of its carry
chain, one more step of the sequencer, would shorten; then the
branch on a register, which a fourth stage, or a register file read
a cycle earlier, would. The lockstep test is the check for any of
these, since it compares architectural state when an instruction
retires.

A bus. The timer sits in the core, decoded from the address beside
the data memory; the next device, whatever it is, should not. A bus
of the core's own, a transaction of address, data and a strobe in
the language's own terms, with the timer moved out onto it as its
first device and the data memory perhaps its second, is the step
that lets the core be one unit among several. It begins in the
model, as everything here has, is added to the core under the
lockstep test, and is checked as a netlist by the same trace.
