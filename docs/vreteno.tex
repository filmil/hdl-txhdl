...
a name and a sentence of its own: the five immediates, the ALU, the
...
memory at \code{0x1000}, the timer at \code{0x2000} and the serial
port at \code{0x3000}, a nibble each, and every other address a hole
...
twenty-four ticks of the demonstration exercise every instruction of
...
program never sets; it is the core's one tile. The data memory, a
...
design meets ten nanoseconds with 0.315~ns of slack, and hold with
0.109~ns. The critical path is 9.095~ns, of which routing is three
quarters, through eleven levels of logic: from the writeback's
destination register, through the forwarding compare into an
operand and the jump's add, to the instruction memory's address, a
jump through a register that the instruction before is still
writing. With the data memory in the core the same path had
...
core halted, a heartbeat, the memory calibrated, and the PLL locked.
...
writes the bitstream, in eleven minutes. Every timing constraint is
met: the worst setup slack is 0.211~ns, the worst hold slack
0.036~ns, over 18\,787 endpoints on the four clocks Vivado derives
...
takes 8\,018 LUTs, 6\,960 flip-flops, three block RAM tiles, four DSP
...
line. So the run on the board is the run in the runtime, ordered the
other way round: the watcher first, then the programming, and the
watcher's log at the end.
...
write software. The core is an RV32IM machine, so a compiler can
...
Three of the core's properties decide how a program has to be
...
The second is the compressed instructions. The core fetches one
32-bit word per instruction and its decode is by opcode, so a
compressed instruction is an illegal opcode, which traps to
\code{mtvec}, which resets to zero, so the program restarts from the
beginning. The only RISC-V target the ruleset maps has the compressed
extension, so codegen is told to leave it out, and
\code{elf2vreteno} refuses an image that holds one rather than trust
the flag.

The third is the size. A program is 1024 instructions, the loader
truncates a longer one without saying so, and the fetch takes
\code{pc[11:2]}, so a program that runs off the end wraps into
itself. Optimising for size is not a preference here. It is also why
debug assertions are off: one of them pulls in \code{core}'s panic
formatting, which is larger than the whole instruction memory.
...
the assembly that has to come before Rust can run at all. And
\code{ebreak} halts this core rather than trapping, which makes it
what a program ends with.
...
because of what comes with it: its \code{rv32im/ilp32} multilib is
this core exactly, RV32IM with no compressed instructions and the
soft-float ABI, and it carries \code{newlib} and a \code{libstdc++}
...
The design with the DDR3 memory on the board: its bitstream is built
and meets timing, and what is left is to program it and see that the
board says \code{ddr3 ok} as the simulation does.
