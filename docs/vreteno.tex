...
Synthesis. The netlist is Verilog and VHDL that simulates; whether
it synthesizes, and to what, is the next question, and the first
answer will come from an open toolchain on an FPGA, with the
instruction memory as a block RAM initialised from the program. The
lowering will need to say what a memory's read port is, since a
combinational read of a block RAM is not what the runtime's plain
read means to every tool.
