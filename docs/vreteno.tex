...
design meets ten nanoseconds with 0.776~ns of slack, and hold with
0.101~ns. The critical path is 8.693~ns, of which routing is three
quarters, through ten levels of logic: from the instruction
register's source field, through the register file's read and the
jump's add, to the instruction memory's address, a jump through a
register. Before the device load's wait moved into writeback the
...
100~MHz with 0.903~ns of slack, in 1992 LUTs and 927 flip-flops
...
move into writeback took the board's slack from 0.285~ns to this,
and a tenth of the LUTs with it.
