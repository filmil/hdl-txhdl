...
\code{nvc} and under Verilator. For the Alinx AX7A200 there is a
...
halves of the MAC. It goes through Vivado's synthesis; four of the
PHY's pins are still to be filled in before it can become a bitstream.
\end{abstract}
...
The Alinx AX7A200 has one gigabit port, a Micrel KSZ9031RNX PHY on
RGMII. RGMII sends a GMII byte as two nibbles a cycle, the low
...
part. It finishes with no error and no critical warning. The design
takes 1341 LUTs, 776 of them memory for the two frames and the FIFO,
and 232 flip-flops, and the estimate after synthesis meets 125~MHz
with 2.388~ns of slack. Its two warnings are that the halves' frame
counters are removed, since nothing on the board reads them, and that
the management pins are driven by a constant or not at all.

The constraints name the clock, the LEDs and the eleven Ethernet pins
that the AX7A200 manual's table gives. Four rows of that table came
through without their pins: \code{ETH\_RXCTL}, \code{ETH\_MDC},
\code{ETH\_MDIO} and \code{ETH\_RESET}. They are in the constraints as
TODO lines, and until they are filled in there is no place and route
and no bitstream.

\lstinputlisting[caption={\code{eth/board/ax7a200\_eth.xdc}: the
board's pins, four still to come.},label={lst:e-xdc}]{eth/board/ax7a200_eth.xdc}

\section{What Is Not Done}
\label{sec:e-not}

The four pins, and the I/O standard of all fifteen Ethernet pins,
which the constraints give as LVCMOS33 like the board's other user
pins, still have to be confirmed. Nothing here has run on the board,
and the receive side's timing at the pins depends on a receive delay
the PHY may or may not add, which has not been measured.

Only gigabit works. At 100 or 10~Mbit the PHY sends a nibble per
cycle, and neither half of the MAC takes a byte over two cycles.
The management interface is left idle, so the PHY runs on its
strapped defaults and nothing reads its link state.
...
\code{buf} or \code{byte}, produces a netlist that does not analyse. A
