...
Its words are stated once, by the \code{regmap!} at the head of
Listing~\ref{lst:e-lite}: \code{status}, read only, with a bit for a
byte waiting and a bit for room to send; \code{txbyte}, write only,
the byte and a bit for the frame's last; and \code{rxbyte}, whose read
takes the byte, with the byte, the last bit and a valid bit.
The same declaration gives the unit its read mux and its enables, and
a program its offsets and fields.
...
\code{ex\_eth} puts the three units behind \code{LiteBridge<1, ..>}, behind
