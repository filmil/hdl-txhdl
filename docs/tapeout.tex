...
Compare that with the same core on the Artix-7, which is 1952 lookup
tables, 486 flip-flops, one block RAM tile and four arithmetic
...
against 486 on the board, where the register file was not flip-flops
at all.

The instruction memory went the other way, and this is the
interesting part. On the FPGA it is one block RAM tile, 1024 words by
27 bits, the tool having dropped the five bit columns the program
never sets. Here there is no block RAM, and the memory is read-only:
