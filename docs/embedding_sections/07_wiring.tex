...
as a wire, and a process in that clock waits for \code{C::edge()}. The
executor runs time in steps and each clock has an edge at the steps its
period and phase name; the waits built on the edge, a channel's
\code{wait} and \code{until}, are in the channel's or the named clock,
