...
the clock: a period and a phase, in a unit common to every clock of
the design. A clock with period 3 and phase 1 has edges at 1, 4, 7 and
so on. The defaults are 1 and 0, so a single-clock design mentions
neither. This is the same statement SDC makes with \code{create\_clock
-period -waveform}, and it was chosen for that reason: it is the form a
constraint file already takes.

A register is then in a clock, \code{Reg<T, C>}, with the same default
as a wire, and reading it waits for the edge of its own clock. The
executor runs time in steps and each clock has an edge at the steps its
period and phase name; a process waits at a register read until the
next edge of that register's clock. Section~\ref{sec:e-regread}'s
rule, one value per register per edge, holds per clock. An operator's
cycle is a cycle of whatever clock the process last crossed an edge of,
which is the only sense a latency has in a process.
...
and the process's type says nothing. That is also the hazard an
...
examples document~\cite{examples}, with a producer clock of period 2
and a consumer clock of period 3 and phase 1; its trace shows the
producer stalled on \emph{full} twice in twenty steps, which is the
backpressure crossing the domains the other way.
