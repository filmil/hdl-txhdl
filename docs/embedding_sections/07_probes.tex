...
and three of them are expected to fail, so they are tagged \code{manual}
...
\code{probe\_funcs}
  ...
  & \textbf{Yes.} An \code{async} operator in \code{crate::funcs} is awaited, and the depth follows from the operators used \\
\code{probe\_if}
  & Does \code{when!} work as expression and as statement?
  & \textbf{Yes}, with the statement form restricted to a list of predicated writes \\
\code{probe\_slice}
...
\subsection{The three that changed the design}
...
Writing \code{when!} showed where the macro route stops, and that is the
