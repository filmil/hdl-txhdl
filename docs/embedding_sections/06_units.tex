...
async fn port_a(&self) {
    loop {
        ...
        let hits = self.hits_a.get();  // a read at that edge
...
an event: the next edge of a clock, \code{C::edge()}; a transaction on
a channel, \code{rx.wait()}; or an edge at which a condition on state
holds, \code{until::<C>(|| ..)}. Once its event has happened it reads
...
no \code{select!}: the first of several channel waits with a
state-dependent priority is \code{until} with an \code{||} in it.
