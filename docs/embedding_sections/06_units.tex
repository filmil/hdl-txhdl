...
rule it keeps took two attempts. The first version made every register
read yield once, so an iteration that read three registers took three
cycles, which no hardware does: a process samples every register at the
same edge. The rule now is that a register gives each process one value
per edge. The first read in an iteration waits for the edge; a read of a
different register after it is free; a read of a register the process
has already read this cycle waits for the next edge, which is what makes
a loop that re-reads its registers advance one iteration per cycle.
Processes are told apart by their waker, which \code{join2} and
\code{join\_all} give each child fresh, so the rule holds per process
rather than per unit. A loop that reads no register and awaits no
