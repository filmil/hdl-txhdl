...
\begin{lstlisting}[caption={A child's constant, reached through the parent, still sizes an array.},label={lst:nestedconst}]
pub const DEPTH: usize =
    <<Fpga as TopConfig>::P as ProducerConfig>::DEPTH;
pub static BUF: [u32; DEPTH] = [0; DEPTH];
\end{lstlisting}

The spelling in that last listing is the one real cost: reaching two
levels down needs a fully qualified path, and three levels reads worse
again. A type alias per level is the usual repair, and a derive on the
configuration could emit them.
