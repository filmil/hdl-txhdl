...
It does not reinterpret \code{if} or \code{match}; it reads a subset
that is also plain Rust, \code{with!}, \code{case!} and
\code{select!} among registers, wires, memories and channel ports,
and refuses the rest with a message of its own. So the cost above,
