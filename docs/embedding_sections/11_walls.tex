...
let chosen = mux(reset, Sig(0), self.count);
\end{lstlisting}
...
makes, and an arm may hold a \code{let}, a \code{when!}, a
\code{case!} or another \code{if}. What the macro keeps is the
arrow, and the guarantee that an arm is a list of drives; what
\code{if} adds is nesting and \code{else if}. Two things stay out,
for the reason above: an \code{if} that yields a value, which is
\code{mux} or \code{select!}, and an output driven under a
condition, which is a wire and takes a \code{mux} outside the chain.
