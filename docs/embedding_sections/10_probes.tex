...
and twelve of them are expected to fail, so they are tagged \code{manual}
...
\code{probe\_if}
  & Does \code{when!} work, with \code{<=} into the register?
  & \textbf{Yes}, as a procedural macro; \code{macro\_rules!} cannot follow an \code{expr} with \code{<=} \\
\code{probe\_when\_bad}
  & A drive with no arrow?
  & \textbf{Refused:} \code{expected `register <= value`} \\
\code{probe\_case}
...
\code{probe\_case\_bad}
  ...
  & \textbf{Refused:} the same message as \code{when!}, from the shared parser \\
