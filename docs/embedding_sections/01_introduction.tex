...
paper type-checks; none \term{lowers} an \code{async fn}, which is to
say none translates it into the registers, wires and control that a
synthesis tool accepts. Lowering is the compiler's word for going down
the abstraction ladder, and it is what turns a value that is alive
across an \code{.await} into a register that the source never named.
