...
The netlist states a unit's statements only while \code{rst} is
low, whether it added the reset port or the unit declared its own, as
the Vreteno core does (issue~633). \code{ex\_rstcheck} shows why: a
counter with its own reset shows a blank while it is held and checks
that what it shows is a digit. Its netlist proves
(\code{//docs:rstcheck\_prove\_test}), and a copy with the gate taken
out, which is what the netlist wrote before, fails its proof and its
bounded search at the first step, in reset
(\code{rstcheck\_ungated\_prove\_test} and
\code{rstcheck\_ungated\_bmc\_test}).

What is still not here is the same rule in the run. The run has no
reset to gate on, so it checks a statement on every edge it is
reached, held in reset or not, and a unit that states something of a
value its reset holds must keep its run's reset low, as
\code{ex\_rstcheck}'s run does.
