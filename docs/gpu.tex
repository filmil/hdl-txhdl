...
it takes to give the GPU work. Both are written in the lowered subset,
...
\caption{The triangle fetched and begun. \code{phase} is one while
the front end reads the instruction's words, which land one at a
time on \code{rdata}, and turns to two as the last word lands; in
that cycle \code{kind} turns from \code{Rect} to \code{Tri}. From
...
works out an icosahedron and writes it into a memory the GPU shares,
