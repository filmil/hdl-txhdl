...
\caption{One instruction ending and the next beginning. \code{kind}
turns from \code{Rect} to \code{Tri} as the rasteriser takes the
entry off \code{op}; from there \code{x} and \code{y} walk the
triangle's box, \code{hit} is whether the pixel is inside it, and
where \code{hit} is low no burst is issued, so the row is walked and
