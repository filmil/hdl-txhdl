...
The peripheral's three words are declared once, with their fields, in
Listing~\ref{lst:h-regs}: \code{status}, read only, the vertical blank
and the count of frames; \code{cursor}, read and written, the column
and the row; and \code{pixel}, written only, the colour.
The same declaration gives the unit its read mux, its write enables
and its field accessors, and a program its offsets and fields.
...
data line comes back in on \code{sda\_in}, where the master reads it at
the end of the acknowledge's second quarter.

The master is two processes. The first is the quarter clock, one
counter that wraps every \code{DIV} cycles, and it carries \code{done}
and \code{failed} out from their registers. The second is the sequence,
written as the transaction reads: the reset held and then left to
settle, each a counted loop of \code{HOLD} waits; then, for each entry
of the table, the start, the three bytes each followed by its
acknowledge, and the stop, every quarter a line set and a wait for the
quarter's end. The lowering numbers the waits into a state register of
its own and gives each counted loop a counter, so the netlist keeps
what a hand-numbered machine would have kept in a phase, a step and a
quarter without the source naming any of them. The sequence ends in a
wait that never returns, since \code{written} stays up.
...
\code{ex\_hdmi} puts the video peripheral behind \code{LiteBridge<1, ..>},
