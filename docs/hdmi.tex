...
The functions come in pairs, one per axis, where one function would
do for both. The lowering binds a function's constant parameters by
name rather than by position, so a function's parameters have to be
named as the unit names its own~\cite{i127}.
...
Nothing here has run on the board. Three things on the board side are
assumptions to be confirmed there.
The chip's register values, as above. The order of the chip's data
inputs: the manual's table numbers them \code{D[0]} to \code{D[23]}
and does not say which are red, green and blue, and the demonstration
puts red on the top eight. And the I/O standard of the chip's pins,
...
here provides. The lowering cannot yet co-simulate a unit of two
clocks, which is what a crossing written in the language would
be~\cite{i131}.
