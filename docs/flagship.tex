...
wire as they are. Its pump is tested against a wire in memory. What
proves the whole of it on the board is issue~\#335's last step, a run
with a cable; until then the proof is the simulation: the core writes
a word to \code{0x3300}, reads it back, and gets what it wrote, with a
program in the test answering the frames.
...
\emph{Nothing on a real wire has answered the remote peripheral yet.}
The design is here, the program is here, and so is the shim between
them, \code{//tools/remote/shim}, with \code{//tools/remote/shim:deploy}
to put it on the machine the board is wired to as a service that is
given the one capability a raw socket wants at launch, so that
nothing is stored on the binary and a redeploy needs no ceremony.
What is not is the run: the service installed there, which is one
sequence as root the deploy prints; the flagship in the part, since
the board's interface has no carrier under any other bitstream; the
program through a tunnel to the service; and the core reading back
what the program computed. That is issue~\#335, and it wants a cable
