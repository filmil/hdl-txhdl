...
Two things the run found. The peripheral waits 20~ms for its answer,
\code{REMOTE\_WAIT}, which is a budget for a program on the board's
own network: with the program on the development machine instead,
175~ms away by round trip through a tunnel, it answered the same
four transactions and the core read zero both times, the bus having
refused before the answer came; that is issue~\#397. And the loader's
serial reset did not bring the loader back after the program had
halted, where a reprogram did; that is issue~\#398.
...
\emph{The remote peripheral waits 20~ms for its answer.} That is a
budget for a program on the board's own network, and it rules out a
program across a tunnel, which the run of Section~\ref{sec:f-remote}
measured; the patience is issue~\#397. \emph{The loader's serial
reset does not return the loader once a program has halted.} A
reprogram does, at a minute instead of a second; that is
issue~\#398.
