...
port there is a shim that puts frames on and off the wire, which is
issue~\#335 and is not written yet. Until it is, what proves this on
the board is the simulation: the core writes a word to \code{0x3300},
reads it back, and gets what it wrote, with a program in the test
answering the frames.
...
The design is here and the program is here, and what is between them
is not: a shim beside the board that reads frames off an interface and
writes them to the stream \code{//tools/remote} speaks, both ways.
That is issue~\#335, and it wants a cable rather than a design.
