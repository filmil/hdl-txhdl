...
100~MHz in 1864 LUTs, its data memory in block RAM. This paper presents the system
and its results.
\end{abstract}
...
Vreteno & the whole core, 520 ticks & nvc, Verilator \\
\bottomrule
\end{tabular}
\end{table}
...
registers. Multiply and divide are one sequencer of thirty-two
shift-and-add or restoring steps over the magnitudes, the signs put
back at the end, and the instruction stalls for them: small, slow,
and off the critical path. The whole is written in the subset the
lowering reads,
...
nanoseconds with 1.013~ns to spare; the critical path runs from the
register file's read through the branch compare to the instruction
memory's address, a branch on a register. Fig.~\ref{fig:p-vreteno} is the opening of the
...
external crate, an FST writer. Twenty-six examples compile, each one
concept, and every one that runs writes a waveform the examples
document draws. Ten lowered units, Table~\ref{tab:p-checked}, are
...
twenty-four tests run under \code{bazel test //...}, and every one of them
is hermetic, the simulators built from source and the synthesis tool
installed by the build. Vreteno is 594 lines of core, checked in
lockstep against a 273-line model on a demonstration program and
sixty-four random ones, checked as a netlist on 520 ticks and eight
...
a multiplier in the DSP blocks; the path that limits the clock is a
branch on what the instruction before it computed.
