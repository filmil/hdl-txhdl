...
them together, and the Vreteno RV32I core built with it. Every
...
largest design so far is Vreteno, a two-stage RV32I core: written in
the subset the lowering reads, checked in lockstep against a
reference model, checked as a netlist against its own trace, and
synthesized on an Artix-7 in 1635 LUTs. This paper presents the
...
the loop. Section~\ref{sec:p-vreteno} is Vreteno, an RV32I core
...
\code{Unit<In, Out>} trait names the ports as type parameters, and
\code{run} takes them as arguments: an input wire and an output wire
here, channels or tuples of either elsewhere. The body is one
...
then it drives, with \code{when!} a predicated register drive and
with \code{set} an output wire. The attribute \code{\#[lower]} says
...
sized vectors \code{U<N>} and \code{I<N>}, with the operators a
datapath needs as plain functions: arithmetic, shifts, bitwise logic,
\code{slice}, \code{concat}, \code{sext} and \code{zext}, and the
compares. A struct of values with \code{derive(Value)} is a value with
...
two others is stated, as \code{concat::<K, M>} states it.
...
reads: a register's \code{get} returns what the last edge latched, a
wire's \code{get} what it carries, a memory's \code{read} its
...
\begin{tikzpicture}[x=1.05cm, y=0.6cm]
  \foreach \t in {0,2,4} {
    \draw[dashed, gray] (\t+1,-0.4) -- (\t+1,4.6);
    \node[font=\tiny, gray] at (\t+1,4.85) {commit};
  }
  ...
  \foreach \t in {0,2,4} {
    ...
    \draw[-{Stealth[length=1.2mm]}, green!50!black] (\t+0.1,0.9) -- (\t+0.9,0.9);
    \node[font=\tiny] at (\t+0.5,0.3) {read, drive};
  }
\end{tikzpicture}
...
A signal has no value while the Rust program runs, so it cannot be
the condition of an \code{if}. Three macros spell what hardware means
instead, and each is named for what it is rather than for the Rust it
resembles. \code{when!(c => \{ r <= v \})} is a predicated drive: both
arms exist, a multiplexer chooses, nothing branches. \code{case!} is
\code{when!} with many arms, Rust patterns with guards and
alternatives, the first match winning, so a state machine is one
\code{case!} over its state register. \code{select!} chooses among
values rather than drives, \code{match} on a value by another name,
and lowers to a chain of multiplexers; \code{mux} is its two-arm
case. Listing~\ref{lst:p-alu} is the Vreteno core's ALU, a
\code{select!} over the function code.
...
\code{\#[lower]} reads a unit's \code{run} loop as it is written and
builds a netlist from it, without a second language: a loop of one
edge wait, register and port reads into names, \code{when!},
...
unit that lowers is the unit that simulates, one file.
...
whose handshake on both sides is what its testbench replays.
...
\midrule
counter & a register, \code{when!}, an output & nvc, Verilator \\
...
Vreteno & the whole core, 66 ticks & nvc, Verilator \\
\bottomrule
\end{tabular}
\end{table}
...
largest design in the language. Fig.~\ref{fig:p-core} is its
...
file, computes with the ALU of Listing~\ref{lst:p-alu} or the
load-store unit, writes the register file and the data memory, and,
on a taken branch or a jump, redirects the fetch and squashes the
word fetched that cycle, which is the one-cycle penalty. The whole is
written in the subset the lowering reads, Listing~\ref{lst:p-fetch}
being the fetch's drives, so the one file simulates and is a netlist.
...
\begin{tikzpicture}[x=1cm, y=1cm]
  ...
  \node[art] (dmem) at (2.3,-3) {data\\memory};
  \node[box] (wb) at (0,-3) {writeback};
  \draw[flow] (pc) -- (imem);
  \draw[flow] (imem) -- (ir);
  \draw[flow] (ir) -- (dec);
  \draw[flow] (dec) -- (regs);
  \draw[flow] (regs) -- (alu.west |- regs.east) -- (alu);
  \draw[flow] (dec) -- (alu);
  \draw[flow] (alu) -- (dmem);
  \draw[flow] (dmem) -- (wb);
  \draw[flow] (wb.north) -- (wb.north |- regs.west) -- (regs.west);
  \draw[flow, red!70!black] (alu.east) -- ++(0.45,0) |- (pc.north) node[lbl, pos=0.25, right] {redirect};
  \begin{scope}[on background layer]
    \node[draw, dashed, gray, fit=(pc)(imem), inner sep=3pt, label={[gray, font=\scriptsize]left:fetch}] {};
    \node[draw, dashed, gray, fit=(dec)(regs)(alu)(dmem)(wb), inner sep=3pt, label={[gray, font=\scriptsize]below:execute}] {};
  \end{scope}
\end{tikzpicture}
\caption{Vreteno: two stages in one process. The instruction register
is the boundary; a taken branch or jump redirects the fetch and costs
one bubble.}
\label{fig:p-core}
\end{figure}

\lstinputlisting[rangebeginprefix=//\ begin\{,rangebeginsuffix=\},rangeendprefix=//\ end\{,rangeendsuffix=\},includerangemarker=false,linerange=fetch-fetch,float,caption={The fetch's drives: reset, halt, redirect or advance, in that priority, one \code{case!}. From \code{core.rs}.},label={lst:p-fetch}]{cpu/vreteno/src/core.rs}
...
thirty-one registers and the halt must agree, and at the halt the
data memory must. The demonstration program and sixty-four random
programs of two hundred instructions pass, and a broken shift or a
branch that does not squash fails at its first use. The netlist is
...
memory and the register file distributed RAM, since their reads are
combinational, and the flip-flops are the five registers.
Fig.~\ref{fig:p-vreteno} is the opening of the demonstration, from
the trace.
...
Flip-flops & 103 \\
Block RAM tiles & 1 \\
Errors, critical warnings, warnings & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
instruction per edge, the writeback decoded into its register and
value, and the first bubble after the first taken branch.}
\label{fig:p-vreteno}
\end{figure}
...
paper. The runtime is five Rust modules and one macro crate, with one
external crate, an FST writer. Twenty-four examples compile, each one
concept, and every one that runs writes a waveform the examples
document draws. Eight lowered units, Table~\ref{tab:p-checked}, are
...
twenty tests run under \code{bazel test //...}, and every one of them
is hermetic, the simulators built from source and the synthesis tool
installed by the build. Vreteno is 268 lines of core, checked in
lockstep against a 169-line model on a demonstration program and
sixty-four random ones, checked as a netlist on 66 ticks and eight
signals, and synthesized in two minutes to the figures of
Table~\ref{tab:p-synth}. Eight documents describe the system, this
one included, and each is built by the same command that builds the
...
trace and synthesizes. The next steps are a place-and-route on a
board, a third pipeline stage for the memories the block RAM wants
registered, and the parts of a unit the lowering does not yet read:
a second process and the falling edge.
