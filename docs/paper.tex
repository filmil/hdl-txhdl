...
machine-mode traps, an interrupt line and a timer: written in the
subset the lowering reads, checked
...
100~MHz in 2169 LUTs, its data memory in block RAM. This paper presents the system
and its results.
\end{abstract}
...
Vreteno & the whole core, 452 ticks & nvc, Verilator \\
\bottomrule
\end{tabular}
\end{table}
...
retires writing nothing; a timer, four words in the address map
beside the memory, raises the second interrupt when its count
reaches its compare; the six CSR instructions read and write the
...
nanoseconds with 0.719~ns to spare; the critical path runs from the
writeback's destination register through the forwarding path, the
address add and the timer's decode to its compare register, a store
to the timer of what the instruction before it computed. Fig.~\ref{fig:p-vreteno} is the opening of the
...
installed by the build. Vreteno is 702 lines of core, checked in
lockstep against a 349-line model on a demonstration program and
sixty-four random ones, with interrupts raised at random, checked as
a netlist on 452 ticks and eight signals, synthesized in two minutes to the figures of
...
a bus of the core's own, with the timer as its first device; the
path that limits the clock is a store to the timer of a forwarded
value, with the multiplier's carry chain close behind.
