...
100~MHz in 2052 LUTs, its data memory in block RAM. This paper presents the system
and its results.
\end{abstract}
...
tap & a send under \code{when!}, a receive with no wait & nvc, Verilator \\
...
Vreteno & the whole core, 458 ticks & nvc, Verilator \\
...
the word read back, on which a timer of sixty lines is the first
device and raises the second interrupt when its count reaches its
compare, the channels between them being the runtime's buffer of two
as hardware; the six CSR instructions read and write the
...
nanoseconds with 0.129~ns to spare; the critical path runs from the
register file's read through the branch compare to the instruction
memory's address, a branch on a register, and is three quarters
routing. Fig.~\ref{fig:p-vreteno} is the opening of the
...
external crate, an FST writer. Twenty-nine examples compile, each one
concept, and every one that runs writes a waveform the examples
document draws. Fourteen lowered units, Table~\ref{tab:p-checked}, are
...
thirty-three tests run under \code{bazel test //...}, and every one of them
is hermetic, the simulators built from source and the synthesis tool
installed by the build. Vreteno is 714 lines of core and 96 of
timer, checked in lockstep against a 354-line model on a
demonstration program and
sixty-four random ones, with interrupts raised at random, checked as
a netlist on 458 ticks and eight signals, synthesized in two minutes to the figures of
Table~\ref{tab:p-synth}, and placed and routed at 100~MHz. Nine documents describe the system, this
...
more devices on the bus, a serial port first; the
path that limits the clock is a branch on a register, with the
multiplier's carry chain close behind.
