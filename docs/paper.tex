...
100~MHz in 1866 LUTs, its data memory in block RAM. This paper presents the system
and its results.
\end{abstract}
...
Vreteno & the whole core, 334 ticks & nvc, Verilator \\
\bottomrule
\end{tabular}
\end{table}
...
registers. Multiply and divide are one sequencer over the
...
nanoseconds with 0.617~ns to spare; the critical path runs from the
...
installed by the build. Vreteno is 593 lines of core, checked in
lockstep against a 273-line model on a demonstration program and
sixty-four random ones, checked as a netlist on 334 ticks and eight
...
the rest of the privileged architecture; the path that limits the
clock is a branch on what the instruction before it computed.
