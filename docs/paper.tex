...
synthesized, placed and routed on an Artix-7 at 100~MHz in 1635
LUTs. This paper presents the system and its results.
\end{abstract}
...
and port reads into names, \code{when!},
...
combinational, and the flip-flops are the five registers. Placed and
routed with the same constraint, the core meets ten nanoseconds with
0.263~ns to spare; the critical path runs from a register through
the decode and the ALU into the register file's write, three quarters
of it routing. Fig.~\ref{fig:p-vreteno} is the opening of the
demonstration, from the trace.
...
instruction per edge with the fetch counter running ahead, the
writeback decoded into its register and value, and the first bubble,
\code{done} low, after the first taken branch.}
\label{fig:p-vreteno}
\end{figure}
...
external crate, an FST writer. Twenty-five examples compile, each one
concept, and every one that runs writes a waveform the examples
document draws. Nine lowered units, Table~\ref{tab:p-checked}, are
...
twenty-two tests run under \code{bazel test //...}, and every one of them
...
signals, synthesized in two minutes to the figures of
Table~\ref{tab:p-synth}, and placed and routed at 100~MHz. Eight documents describe the system, this
...
trace, and runs at 100~MHz on an Artix-7, whose bitstream, with a
clock, a reset and four LEDs around the core, is built. The next
steps are the board in hand, and a third pipeline stage for the
memories the block RAM wants registered.
