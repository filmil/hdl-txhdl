...
interrupt when its count reaches its compare and a serial port says
what the program writes, the channels between them all being the
runtime's buffer of two as hardware; the six CSR instructions read and write the
...
document draws. Sixteen lowered units, Table~\ref{tab:p-checked}, are
...
thirty-seven tests run under \code{bazel test //...}, and every one of them
is hermetic, the simulators built from source and the synthesis tool
installed by the build. Vreteno is 725 lines of core, 98 of timer,
...
minutes to the figures of Table~\ref{tab:p-synth}, and placed and
routed at 100~MHz. Nine documents describe the system, this one
...
the serial port's receive side; the path that limits the clock is a
branch on a register, with the multiplier's carry chain close
behind.
