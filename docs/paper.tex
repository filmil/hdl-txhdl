...
machine-mode traps and an interrupt line: written in the subset the
...
100~MHz in 1950 LUTs, its data memory in block RAM. This paper presents the system
and its results.
\end{abstract}
...
Vreteno & the whole core, 366 ticks & nvc, Verilator \\
\bottomrule
\end{tabular}
\end{table}
...
retires writing nothing; the six CSR instructions read and write the
...
nanoseconds with 0.534~ns to spare; the critical path is the
multiplier, from the sequencer's registers into the pair through four
DSP blocks and a carry chain, and the next is a branch on a
register. Fig.~\ref{fig:p-vreteno} is the opening of the
...
installed by the build. Vreteno is 643 lines of core, checked in
lockstep against a 304-line model on a demonstration program and
sixty-four random ones, with interrupts raised at random, checked as
a netlist on 366 ticks and eight signals, synthesized in two minutes to the figures of
...
a timer on the data bus; the path that limits the clock is the
multiplier's carry chain, with a branch on a register close behind.
