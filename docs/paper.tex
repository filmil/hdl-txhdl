...
them together, and the Vreteno RV32IMC core built with it. Every
...
largest design so far is Vreteno, a three-stage RV32IMC core with
...
the loop. Section~\ref{sec:p-vreteno} is Vreteno, an RV32IMC core
...
an input wire and an output wire here, channels, or tuples or
structs of either, elsewhere. The body is one
...
\begin{table}[htbp]
\caption{Lowered units simulated against their traces, each under nvc and under Verilator.}
\label{tab:p-checked}
\centering
\footnotesize
\begin{tabular}{@{}ll@{}}
...
Unit & What it shows \\
\midrule
counter & a register, \code{with!}, an output \\
sampler & two processes, one per edge \\
blinky & a configuration's constants baked in \\
sequencer & \code{case!} on a state enum, guards \\
ALU & ten operators by \code{case!} \\
scratchpad & a memory written and read \\
decoder & \code{slice}, \code{concat}, \code{sext}, \code{select!} \\
stage & two channels, the handshake \\
MAC & \code{mul::<M>}, a product at a stated width \\
tap & a send under \code{if}, a receive with no wait \\
buffer & the runtime's channel, as a part \\
divider & wires kept as fields, seen and checked \\
Gray counter & functions under \code{\#[lower]}, inlined \\
Vreteno & the whole core, 647 ticks \\
router & the bus fanned out by address \\
data memory & on the core's bus, the same run \\
timer & on the core's bus, the same run \\
serial port & on the core's bus, the same run \\
FIFO & \code{D} words between two channels \\
station & two tagged inputs, beside hand Verilog \\
stream ALU & four latencies, results out of order \\
top & a unit of units, a station and the ALU \\
\bottomrule
\end{tabular}
\end{table}
...
address range, on which the core-local interruptor raises the second
interrupt when its count reaches its compare and the third when a
program writes its own software bit, a serial port says what the program writes and takes what
...
memory became a read-only memory in two block RAM tiles, one for
each of the two words the fetch reads, since an instruction may cross
from one into the next, the data
...
nanoseconds with 0.296~ns to spare; the critical path runs from a
register field of the instruction register through an operand's
read and its forwarding into the ALU's compare and the writeback
value, and is three quarters routing. Fig.~\ref{fig:p-vreteno} is the opening of the
...
The same netlist goes the other way as well. Mapped onto Nangate45,
an open 45~nm standard cell library, by Yosys, and then floorplanned,
placed, routed and timed by OpenROAD, both of them fetched by
checksum as Debian packages and unpacked by the build, it comes to
\asiccells{} cells on a die \asicdie~$\mu$m square, meets
\asicperiod~ns with \asicslack~ns to spare, and routes with
\asicdrc{} violations. What changes between the two targets is the
memories. An FPGA has a block RAM to put the instruction memory in
and a small distributed memory to put the register file in, and a
standard cell library has neither: the instruction memory, which
nothing in the core writes, becomes a mask ROM of gates whose size
depends on the program in it, and the register file becomes a
thousand and twenty four flip-flops. That is why the same core is 489
flip-flops on the board and \asicflops{} on silicon. The flow, the
layout and everything a tapeout would still want are
in~\cite{tapeout}.
...
\subsection{Programs written for it}

A core that runs only what its own assembler emits is a core that has
been tested, not one that can be programmed. Vreteno is RV32IMC, so
compilers target it, and two do: Rust through the ruleset's own
toolchain, and C++ through the GNU bare-metal RISC-V toolchain, whose
\code{rv32imc/ilp32} multilib is this machine exactly and carries
\code{newlib} and a \code{libstdc++} built for it. Both are fetched
by the build against a checksum. Nothing is installed to compile for
the core, as nothing is installed to simulate or to synthesise it.

A program for the core is built for a different machine than the
build runs on, so a rule with a platform transition carries it
across, and \code{//tools/elf2vreteno} turns the linked ELF into the
two images the core is loaded from. Both languages end at the same
place: the same machine, the same terminal on the serial line, the
same assertion that the line came out and that the core stopped
itself.

What is interesting is not that it works but what the machine will
not allow, because the core meets each restriction by going quietly
wrong rather than by refusing, so each had to become a check that
speaks. A load below the data base never leaves the core, so a
constant placed beside the code is one the program cannot read and
the load retires with a stale word; \code{.rodata} is therefore
linked into the data memory and the image carries its bytes, and
there is no startup copy that could do it instead, since the copy
could not read the source either. And the instruction memory is
4~KiB, which the loader truncates to without a word and the fetch
wraps within. Each is refused by the image tool, by name, where it
can be said out loud. The compressed instructions were a third such
restriction until the core ran them; now the compilers' own targets
are the core's, and the programs are a quarter smaller for it.

That size is the real constraint on what a standard library can mean
here. The half of it that is templates and constants is usable and
both programs use it; the half that needs an operating system
underneath, a heap, file descriptors, an unwinder, is not, and no
amount of toolchain fetches it into existence. The C++ program says
hello and prints a number its compiler worked out, in 63 words.
...
installed by the build. Vreteno is 1270 lines of core, 141 of data
memory, 168 of interrupt controller and 253 of serial port, with the
router a family the macro writes in the parts library rather than a
file of its own, checked in lockstep against a 572-line model on a
demonstration program and sixty-four random ones, with interrupts raised at random, checked as a netlist on 647
...
hardware, end to end, from a Rust source to a board that answers. Sixteen documents
...
\bibitem{tapeout}
F.~Filmar and D.~Janković, ``A tapeout prep in TxHDL: Vreteno on an
open 45~nm node,'' project repository \code{HDL/txhdl},
\code{//docs:tapeout}, 2026.
