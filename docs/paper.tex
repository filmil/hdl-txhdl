...
largest design so far is Vreteno, a three-stage RV32I core: written in
...
synthesized, placed and routed on an Artix-7 at 100~MHz in 1040
LUTs, its data memory in block RAM. This paper presents the system
and its results.
\end{abstract}
...
file, computes with the ALU of Listing~\ref{lst:p-alu}, stores into
the lanes of the data memory, reads it into the writeback registers
at its edge, and, on a taken branch or a jump, redirects the fetch
...
to write is forwarded into execute, so no stall is needed. The whole
is written in the subset the lowering reads, Listing~\ref{lst:p-fetch}
...
branch or jump redirects the fetch and costs one bubble; the value
in writeback is forwarded into execute.}
\label{fig:p-core}
\end{figure}
...
nanoseconds with 0.361~ns to spare; the critical path runs from a
data memory's read through the forwarding path and the ALU to the
instruction memory's address, a load followed by a jump through what
it loaded. Fig.~\ref{fig:p-vreteno} is the opening of the
...
trace, and runs at 100~MHz on an Artix-7 with its data memory in
block RAM, whose bitstream, with a clock, a reset and four LEDs
around the core, is built. The next steps are the board in hand, and
the clock: the path that limits it is a load followed by a jump
through what it loaded.
