...
largest design so far is Vreteno, a three-stage RV32I core with
machine-mode traps: written in the subset the lowering reads, checked
in lockstep against a reference model, checked as a netlist against
its own trace, and synthesized, placed and routed on an Artix-7 at
100~MHz in 1297 LUTs, its data memory in block RAM. This paper presents the system
and its results.
\end{abstract}
...
Vreteno & the whole core, 180 ticks & nvc, Verilator \\
\bottomrule
\end{tabular}
\end{table}
...
returns; the six CSR instructions read and write the five control
registers. The whole is written in the subset the lowering reads,
...
nanoseconds with 0.737~ns to spare; the critical path runs from a
...
installed by the build. Vreteno is 490 lines of core, checked in
lockstep against a 246-line model on a demonstration program and
sixty-four random ones, checked as a netlist on 180 ticks and eight
...
the multiplication extension; the path that limits the clock is now
a branch on what the instruction before it computed.
