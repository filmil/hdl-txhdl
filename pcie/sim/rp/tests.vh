...
// bridge has none of. The arm reads BAR0 through to `BarRegs`: the
...
else if (testname == "bar0")
begin
    $display("[%t] : bar0: BAR0 is at %h, link_up is %b", $realtime,
             BAR_INIT_P_BAR[0][31:0], board.EP.top.link_up);
    ...
    fork
        TSK_TX_MEMORY_READ_32(DEFAULT_TAG, DEFAULT_TC, 11'd1,
                              BAR_INIT_P_BAR[0][31:0] + 32'h0, 4'h0, 4'hF);
        TSK_WAIT_FOR_READ_DATA;
    join
    DEFAULT_TAG = DEFAULT_TAG + 1;
    $display("[%t] : bar0: word 0 reads %h", $realtime, P_READ_DATA);
    ...
    fork
        TSK_TX_MEMORY_READ_32(DEFAULT_TAG, DEFAULT_TC, 11'd1,
                              BAR_INIT_P_BAR[0][31:0] + 32'h4, 4'h0, 4'hF);
        TSK_WAIT_FOR_READ_DATA;
    join
    DEFAULT_TAG = DEFAULT_TAG + 1;
    $display("[%t] : bar0: word 1 reads %h", $realtime, P_READ_DATA);
    ...
    TSK_TX_MEMORY_WRITE_32(DEFAULT_TAG, DEFAULT_TC, 11'd1,
                           BAR_INIT_P_BAR[0][31:0] + 32'h8, 4'h0, 4'hF, 1'b0);
    TSK_TX_CLK_EAT(100);
    DEFAULT_TAG = DEFAULT_TAG + 1;
    ...
    fork
        TSK_TX_MEMORY_READ_32(DEFAULT_TAG, DEFAULT_TC, 11'd1,
                              BAR_INIT_P_BAR[0][31:0] + 32'h8, 4'h0, 4'hF);
        TSK_WAIT_FOR_READ_DATA;
    join
    DEFAULT_TAG = DEFAULT_TAG + 1;
    $display("[%t] : bar0: scratch reads %h", $realtime, P_READ_DATA);
    ...
    fork
        TSK_TX_MEMORY_READ_32(DEFAULT_TAG, DEFAULT_TC, 11'd1,
                              BAR_INIT_P_BAR[0][31:0] + 32'h18, 4'h0, 4'hF);
        TSK_WAIT_FOR_READ_DATA;
    join
    DEFAULT_TAG = DEFAULT_TAG + 1;
    $display("[%t] : bar0: writes reads %h", $realtime, P_READ_DATA);
    if (P_READ_DATA !== 32'h0000_0001) testError = 1'b1;

    board.ok = ~testError;
    $display("[%t] : bar0: %s", $realtime, testError ? "FAILED" : "PASSED");
    $finish;
end
