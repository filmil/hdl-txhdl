...
// and `elaborate`.
use txhdl::comp::{elaborate, Config, Module, Reg, edge, DefaultClock};
use txhdl::types::U;
...
impl<const TAPS: usize> Module<(), ()> for Top<TAPS> {
    async fn run(&mut self, _i: (), _o: ()) {
        loop { for i in 0..TAPS { edge::<DefaultClock>().await; let a = self.acc.get(); self.acc.set(a.wrapping_add(U::new(i as u128 * 2))) } }
    }
}
