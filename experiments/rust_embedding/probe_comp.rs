...
use txhdl::comp::{chan, signal, DefaultClock, In, Module, Out, Rx, Tx};
...
impl<T: Tag> Module<U<32>, ()> for MacUnit<T> {
    async fn run(&mut self, addr: U<32>, _o: ()) {
        ...
        if self.ack.get().raw() == 1 { self.acc = self.acc.wrapping_add(U::new(1)) }
    }
}
...
pub fn build() -> (MacUnit<MemFetch>, Rx<MemRead>, Out<U<1>>) {
    ...
    (MacUnit { req: tx, ack: ack_in, acc: U::new(0), _t: core::marker::PhantomData }, rx, ack_out)
}
