...
//!
//! The core, its tracker, a router, and four peripherals, each behind a
//! tracker of its own or, for the serial port, the AXI-Lite bridge: the
//! data memory at `0x1000`, the timer at `0x2000`, the
//! serial port at `0x3000`, and the board's DDR3 memory from
//! `0x4000_0000` to the end of the first two gigabytes. `run.rs` wires
...
//! ports. `MICRON_SIM` and `BIST` are the controller's, and `DIV` is
//! the serial port's clock divider.
...
/// the quarter of the address space from `0x4000_0000`.
pub type BoardRouter = Router4<
    32,
    32,
    4,
    2,
        ...
        join2(
            join2(
                ...
                join2(
                    ...
                    self.cpu.run(
                        (rst, irq, tirq_i, rdata_rx, done_rx, grant_rx),
                        (
                ...
                join2(
                    ...
                    self.router.run(
                        (
                            ...
                            b2_rx, r2_rx, b3_rx, r3_rx,
                        ),
                        (
                            aw0_tx, ar0_tx, w0_tx, aw1_tx, ar1_tx, w1_tx,
                            aw2_tx, ar2_tx, w2_tx, aw3_tx, ar3_tx, w3_tx, b_tx,
                            r_tx,
                        ),
                    ),
                ),
