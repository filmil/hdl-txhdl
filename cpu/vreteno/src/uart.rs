    ...
    for Uart<DIV>
{
    async fn run(
        ...
        loop {
            ...
            let hit = eq(addr.slice::<3, 29>(), U::from(UART_BASE >> 3));
            let sel = addr.bit(2);
            let we = r.bit(0);
            let word = mux(sel, busy.zext::<32>(), last.zext::<32>());
            let write = offered.and(we).and(hit);
            let read = offered.and(we.not());
            // A byte written while idle starts a frame: a start bit, the
            // byte, a stop bit, ten bits of DIV cycles each.
            let start = write.and(sel.not()).and(busy.not());
            let octet = r.slice::<REQ_WDATA, 8>();
            let frame = U::<1>::from(1u8)
                .concat::<8, 9>(octet)
                .concat::<1, 10>(U::<1>::from(0u8));
            let last_cycle = eq(tick, U::<32>::from(DIV - 1));
            when!(rst => {
            ...
            when!(rst.not().and(busy).and(last_cycle.not()) => {
                self.tick <= tick.wrapping_add(U::<32>::from(1u32))
            });
            when!(rst.not().and(busy).and(last_cycle) => {
                self.tick <= U::from(0u8);
                self.bits <= bits.wrapping_sub(U::from(1u8));
                self.shift <= U::<1>::from(1u8)
                    .concat::<9, 10>(shift.slice::<1, 9>())
            });
